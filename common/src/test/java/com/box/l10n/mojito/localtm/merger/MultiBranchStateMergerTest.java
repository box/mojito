...
public class MultiBranchStateMergerTest {
  ...
  static Stream<Arguments> merge() {
    ...
    BranchStateTextUnit state1BranchStateTextUnit2 =
        createBranchStateTextUnit("MD5HASH2", createBranchMap(branch1));

    MultiBranchState state1 =
        MultiBranchState.of()
            .withBranches(ImmutableSet.of(branch1, branch2))
            .withBranchStateTextUnits(
                ImmutableList.of(state1BranchStateTextUnit1, state1BranchStateTextUnit2));

    BranchStateTextUnit state2BranchStateTextUnit2 =
        createBranchStateTextUnit("MD5HASH2", createBranchMap(branch3));

    MultiBranchState state2 =
        MultiBranchState.of()
            .withBranches(ImmutableSet.of(branch3))
            .withBranchStateTextUnits(ImmutableList.of(state2BranchStateTextUnit2));

    BranchStateTextUnit merged1BranchStateTextUnit1 =
        createBranchStateTextUnit("MD5HASH1", createBranchMap(branch1, branch2));

    BranchStateTextUnit merged1BranchStateTextUnit2 =
        createBranchStateTextUnit("MD5HASH2", createBranchMap(branch3, branch1));

    MultiBranchState merge1 =
        MultiBranchState.of()
            .withBranches(ImmutableSet.of(branch3, branch1, branch2))
            .withBranchStateTextUnits(
                ImmutableList.of(merged1BranchStateTextUnit1, merged1BranchStateTextUnit2));

    MultiBranchState merge1Priority =
        MultiBranchState.of()
    ...
    return Stream.of(
        Arguments.of(
            ...
            ImmutableSet.of(),
            MultiBranchState.of()),
        Arguments.of("States with basic data", state1, state2, ImmutableSet.of(), merge1),
        Arguments.of(
  ...
  static BranchStateTextUnit createBranchStateTextUnit(
      String md5HASH1, ImmutableMap<String, BranchData> branchMap) {
  ...
  static ImmutableMap<String, BranchData> createBranchMap(Branch... branches) {
    return Arrays.stream(branches)
        .collect(ImmutableMap.toImmutableMap(Branch::getName, b -> BranchData.of()));
  }
  ...
  static Stream<Arguments> sortBranchesByPriorityThenCreatedDateThenName() {
    ...
    return Stream.of(
        ...
        Arguments.of(
            "1 branch", ImmutableSet.of(branch1), ImmutableSet.of(), ImmutableSet.of(branch1)),
        Arguments.of(
  ...
  static Stream<Arguments> mergerMultiBranchStateIntoBranchStateTextUnit() {
    ...
    MultiBranchState toMergeStateWithMatch =
        MultiBranchState.of()
            .withBranches(ImmutableSet.of(branch2, branch3))
            .withBranchStateTextUnits(ImmutableList.of(match));

    MultiBranchState toMergeStateWithNewUsage =
        MultiBranchState.of()
            .withBranches(ImmutableSet.of(branch2, branch3))
            .withBranchStateTextUnits(
                ImmutableList.of(
                    createBranchStateTextUnit(
                        "MD5HASH",
                        ImmutableMap.of(
                            "branch2", BranchData.of().withUsages(ImmutableSet.of("new usage"))))));
  ...
  static Stream<Arguments> sortBranchDataByBranch() {
    ...
    return Stream.of(
        ...
        Arguments.of(
            "1 branch", createBranchMap(branch1), ImmutableSet.of(), createBranchMap(branch1)),
        Arguments.of(
  ...
  public void merge(
      ...
      MultiBranchState toMerge,
      ImmutableSet<String> priorityBranchNames,
      MultiBranchState expected) {
  ...
  public void mergeBranchesByPriorityThenCreatedDateThenName(
    ...
    ImmutableSet<Branch> branches =
        multiBranchStateMerger.mergeBranchesByPriorityThenCreatedDateThenName(
            branch, branch2, priorities);

    assertThat(branches.asList())
        .as(message)
        .usingRecursiveComparison()
        .isEqualTo(expectedBranch.asList());
  }
  ...
  public void sortBranchesByPriorityThenCreatedDateThenName(
    ...
    ImmutableSet<Branch> result =
        multiBranchStateMerger.sortBranchesByPriorityThenCreatedDateThenName(
            branches, sortedBranchNames);
  ...
  public void sortBranchDataByBranch(
    ...
    MultiBranchStateMerger multiBranchStateMerger = new MultiBranchStateMerger();
    ImmutableMap<String, BranchData> result =
        multiBranchStateMerger.sortBranchDataByBranch(branchData, sortedBranches);
    assertThat(result.keySet().asList())
        .as(message)
        .usingRecursiveComparison()
        .isEqualTo(expected.keySet().asList());
  }
  ...
  public void mergerMultiBranchStateIntoBranchStateTextUnit(
    ...
    MultiBranchStateMerger multiBranchStateMerger = new MultiBranchStateMerger();
    BranchStateTextUnit result =
        multiBranchStateMerger
            .mergerMultiBranchStateIntoBranchStateTextUnit(toMergeState, ImmutableSet.of())
            .apply(baseBranchStateTextUnit);
