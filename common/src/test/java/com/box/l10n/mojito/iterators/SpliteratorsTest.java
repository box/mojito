...
public class SpliteratorsTest {
  ...
  public void partitionStream() {
    ...
    List<List<Integer>> partitions =
        Spliterators.partitionStream(IntStream.range(0, sourceSize).spliterator(), partitionSize)
  ...
  public void partitionStreamWithPageFetcher() {
    ...
    PageFetcherOffsetAndLimitSplitIterator<String> spliterator =
    ...
    ImmutableList<List<String>> partitions =
        Spliterators.partitionStream(spliterator, partitionSize)
            .peek(strings -> checkCallOrders.add("chunk-" + strings.size()))
            .collect(ImmutableList.toImmutableList());

    Assertions.assertThat(checkCallOrders)
        .isEqualTo(
            ImmutableList.of(
                "spliterator-0-5",
                "chunk-3",
    ...
    Assertions.assertThat(partitions)
        .isEqualTo(
            Arrays.asList(
                Arrays.asList("str-0", "str-1", "str-2"),
