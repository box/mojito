...
public class JsonObjectRemoverByValue {
  ...
  /**
   * Removes the objects that contain the specified value from the input JSON and returns the
   * indented result.
   *
   * <p>- if the object: o1 that contains the specified value is in an array: a1 then o1 is removed
   * from a1 - if the object: o1 that contains the specified value is in another object: o2 (for
   * attribute name: an1) then the field: "an1 : o1" is removed from o2
   *
   * @param jsonContent
   * @param valueToRemove
  ...
  static void removeFieldsWithValueFromObject(ObjectNode objectNode, String valueToRemove) {
    List<String> toRemove =
        Streams.stream(objectNode.fields())
            .filter(
                f ->
                    isTextutalFieldWithValue(valueToRemove, f.getValue())
