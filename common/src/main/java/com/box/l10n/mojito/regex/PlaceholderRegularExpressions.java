...
public enum PlaceholderRegularExpressions {
  ...
  PRINTF_LIKE_VARIABLE_TYPE_REGEX(
      "%([{|(])+([a-zA-Z0-9_])+([}|)])+([-#+ 0,\\.(\\<]+)?(\\d+)?(\\.\\d+)?([tT])?(hh|h|l|ll|j|z|t|L)?(%|c|d|e|E|f|F|g|G|i|o|p|r|s|u|x|X)"),
  ...
  PLACEHOLDER_IGNORE_PERCENTAGE_AFTER_BRACKETS(
      "(?<![}|)])%(\\d+\\$)?([-#+ 0,(\\<]*)?(\\d+)?(\\.\\d+)?([tT])?(hh|h|l|ll|j|z|t|L)?(%|d|i|u|o|x|X|f|F|e|E|g|G|a|A|c|s|p|n|@)"),

  /**
   * Extension of PRINTF_LIKE_REGEX that handles placeholders with specifiers after '@' e.g.
   * '%1$@ld'
   */
