...
public class Evolve {
  ...
  public Stream<Course> getCourses() {
    ...
    PageFetcherCurrentAndTotalPagesSplitIterator<Course> iterator =
        new PageFetcherCurrentAndTotalPagesSplitIterator<>(
            pageToFetch -> {
              Courses courses =
                  restTemplate.getForObject(
                      "courses?currentPage={currentPage}", Courses.class, pageToFetch);
  ...
  public void createCourseTranslationsById(
      String courseId, String translatedCourse, String locale, boolean isRTL) {
    ...
    String response =
        restTemplate.postForObject("translate/{courseId}", httpEntity, String.class, courseId);
