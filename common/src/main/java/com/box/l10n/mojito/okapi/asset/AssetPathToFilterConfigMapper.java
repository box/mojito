...
/**
 * Helper class to map an asset type to a filter config. This allows to create a generic pipeline
 * that can process different types of asset.
 *
 * @author aloison
 */
@Component
public class AssetPathToFilterConfigMapper {
  ...
  public static final String MACSTRINGS_FILTER_CONFIG_ID =
      MacStringsFilter.FILTER_CONFIG_ID + "-macStrings";
   ...
   * @throws UnsupportedAssetFilterTypeException If cannot find a suitable filter configuration for
   *     the asset
   */
  ...
  private AssetFilterType getAssetFilterTypeFromPath(String assetPath)
    ...
    if (assetFilterType == null) {
      ...
      throw new UnsupportedAssetFilterTypeException(
          "Cannot find a suitable filter configuration for the asset: " + assetPath);
    }
