...
public class FormLoginAuthenticationCsrfTokenInterceptor implements ClientHttpRequestInterceptor {
    ...
    /**
     * The {@link AuthenticatedRestTemplate} to which this interceptor is being
     * used.
     */
    @Autowired
    ...
    /**
     * This is so that we obtain access to the cookie store used inside
     * HttpClient to check to see if we have a session
     */
    CookieStore cookieStore;

    /**
     * This is the latest session id that was used to obtain the
     * {@link this#latestCsrfToken}
     */
    String latestSessionIdForLatestCsrfToken;

    /**
     * This is the lastest CSRF token that was obtained from the
     * {@link this#latestSessionIdForLatestCsrfToken}
     */
    ...
    /**
     * This intercepts a request then will either use the existing session, or
     * will authenticate to obtain a proper session, or will inject the request
     * with the correct CSRF token.
     * <p/>
     * Flow example: 1. GET /login - because it has not been prepped, it starts
     * auth flow 2. POST /login 3. GET /session-timeout-endpoint - response
     * 302/login 4. GET /login 5. POST /login 6. GET /session-timeout-endpoint
     *
     * @param request
     * @param body
     * @param execution
    ...
    /**
     * Handle http response from the intercept. It will check to see if the
     * initial response was successful (ie. error status such as 301, 403). If
     * so, it'll try the authentication flow again. If it further encounters an
     * unsuccessful response, then it'll throw a {@link RestClientException}
     *
     * @param request
     * @param body
     * @param execution
    ...
    /**
     * Gets the authenticated session id. If it is not found, an authentication
     * flow will be started so that a proper session id is available
     *
     * @return
     */
    protected boolean doesSessionIdInCookieStoreExistAndMatchLatestSessionId() {
        logger.debug("Check to see if session id in cookie store matches the session id used to get the latest CSRF token");
