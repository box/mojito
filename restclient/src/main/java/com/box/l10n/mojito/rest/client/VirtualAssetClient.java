...
public class VirtualAssetClient extends BaseClient {
  ...
  public VirtualAsset createOrUpdate(VirtualAsset virtualAsset) {
    return authenticatedRestTemplate.postForObject(
        getBasePathForEntity(), virtualAsset, VirtualAsset.class);
  }
  ...
  public PollableTask addTextUnits(long assetId, List<VirtualAssetTextUnit> virtualAssetTextUnits) {
    ...
    return authenticatedRestTemplate.postForObject(
        uriString, virtualAssetTextUnits, PollableTask.class);
  }

  public PollableTask repalceTextUnits(
      long assetId, List<VirtualAssetTextUnit> virtualAssetTextUnits) {
    String uriString = getVirtualTextUnitsUri(assetId);
    return authenticatedRestTemplate.putForObject(
        uriString, virtualAssetTextUnits, PollableTask.class);
  }

  public PollableTask importTextUnits(
      long assetId, long localeId, List<VirtualAssetTextUnit> virtualAssetTextUnits) {
  ...
  public List<VirtualAssetTextUnit> getLocalizedTextUnits(
      long assetId, long localeId, String inheritanceMode) {
    UriComponentsBuilder uriComponentsBuilder =
        UriComponentsBuilder.fromPath(getBasePathForEntity())
            .pathSegment("{assetId}", "locale", Long.toString(localeId), "textUnits");
    uriComponentsBuilder.queryParam("inheritanceMode", inheritanceMode);
    String uriString = uriComponentsBuilder.buildAndExpand(assetId, localeId).toUriString();
    VirtualAssetTextUnit[] virtualAssetTextUnits =
        authenticatedRestTemplate.getForObject(uriString, VirtualAssetTextUnit[].class);
    return Arrays.asList(virtualAssetTextUnits);
  }

  String getVirtualTextUnitsUri(long assetId) {
    UriComponentsBuilder uriComponentsBuilder =
        UriComponentsBuilder.fromPath(getBasePathForEntity()).pathSegment("{assetId}", "textUnits");
