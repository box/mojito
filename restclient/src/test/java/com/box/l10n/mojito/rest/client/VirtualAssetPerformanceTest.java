    ...
    void importTranslations(Repository repository, VirtualAsset virtualAsset) {
        ...
        repository.getRepositoryLocales().stream()
                ...
                .forEach(rl -> {
                    ...
                    List<VirtualAssetTextUnit> toImport = IntStream.range(0, NUMBER_OF_TEXTUNITS).mapToObj(idx -> {
                        VirtualAssetTextUnit virtualAssetTextUnit = new VirtualAssetTextUnit();
                        virtualAssetTextUnit.setName("name-" + idx);
                        virtualAssetTextUnit.setContent("content-" + rl.getLocale().getBcp47Tag() + "-" + idx + "-" + Strings.padStart("", 30, 'a'));
                        return virtualAssetTextUnit;
                    }).collect(Collectors.toList());

                    PollableTask pollableTask = virtualAssetClient.importTextUnits(virtualAsset.getId(), rl.getLocale().getId(), toImport);
                    pollableTaskClient.waitForPollableTask(pollableTask.getId());
                    pollableTask = pollableTaskClient.getPollableTask(pollableTask.getId());
                    logger.debug("import {}: {}", rl.getLocale().getBcp47Tag(), getElapsedTime(pollableTask));
                });
    }
