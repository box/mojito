...
## Shared HTTP proxy (Box SDK, AI translation, AI review)

Prefer a single shared proxy for outbound Box Public API and OpenAI traffic:

    l10n.webproxy.host=proxy.example.com
    l10n.webproxy.port=3128
    l10n.webproxy.user=proxy-user
    l10n.webproxy.password=${PROXY_PASSWORD}

    # Opt-in when the proxy requires Basic auth on HTTPS CONNECT.
    # The JDK disables Basic for tunneling by default.
    l10n.webproxy.allowBasicTunneling=true

Per-service settings still work and take precedence when host and port are both set:

- `l10n.boxclient.proxyHost` / `proxyPort` / `proxyUser` / `proxyPassword`
- `l10n.ai-translate.proxy-host` / `proxy-port` / `proxy-user` / `proxy-password`
- `l10n.ai-review.proxy-host` / `proxy-port` / `proxy-user` / `proxy-password`

`allowBasicTunneling` is read only by the OpenAI HTTP client (AI translation/review). The Box SDK uses its own proxy Basic auth and does not need this flag. The prefix is lowercase (`webproxy`, like `boxclient`) because Spring Boot requires `@ConfigurationProperties` prefixes to be canonical; field names remain camelCase.

## AI Translation and AI Review (OpenAI)

AI translation and AI review use the OpenAI HTTP API. Configure an API token and, when required by your network, the shared HTTP proxy (or optional per-service overrides above).
