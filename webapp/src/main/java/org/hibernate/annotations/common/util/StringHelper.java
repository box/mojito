...
public final class StringHelper {
  ...
  public static String replace(
      String template, String placeholder, String replacement, boolean wholeWords) {
    if (template == null) {
      return template;
    }
  ...
  /**
   * Collapses a name. Mainly intended for use with classnames, where an example might serve best to
   * explain. Imagine you have a class named <samp>'org.hibernate.util.StringHelper'</samp>; calling
   * collapse on that classname will result in <samp>'o.h.u.StringHelper'<samp>.
   *
   ...
   * @return The name itself, or the partially unqualified form if it begins with the qualifier
   *     base.
   */
  ...
  /**
   * Cross between {@link #collapse} and {@link #partiallyUnqualify}. Functions much like {@link
   * #collapse} except that only the qualifierBase is collapsed. For example, with a base of
   * 'org.hibernate' the name 'org.hibernate.util.StringHelper' would become
   * 'o.h.util.StringHelper'.
   *
   * @param name The (potentially) qualified name.
   * @param qualifierBase The qualifier base.
   * @return The name itself if it does not begin with the qualifierBase, or the properly collapsed
   *     form otherwise.
   */
   ...
   * Subclasses of Loader do <em>not</em> have to use aliases of this form.
   *
  ...
  /**
   * Generates a root alias by truncating the "root name" defined by the incoming decription and
   * removing/modifying any non-valid alias characters.
   *
  ...
  /**
   * Return a representation of the given name ensuring quoting (wrapped with '`' characters). If
   * already wrapped return name.
   *
