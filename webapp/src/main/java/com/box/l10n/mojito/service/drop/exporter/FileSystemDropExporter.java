...
public class FileSystemDropExporter implements DropExporter {
  ...
  public void setConfig(String config)
    ...
    try {
      fileSystemDropExporterConfig =
          objectMapper.readValue(config, FileSystemDropExporterConfig.class);
  ...
  public void init(String dropGroupName, String dropName)
    ...
    if (fileSystemDropExporterConfig != null) {
      logger.debug(
          "fileSystemDropExporterConfig was set, the filter is already initialized and ready to be used");
    } else {
   ...
   * #fileSystemDropExporterConfig} accordingly.
   *
  ...
  private File getDropGroupFolder(String dropGroupName) {
    ...
    File dropGroupFolder =
        fileSystemDropExporterConfigFromProperties.getBasePath().resolve(folderName).toFile();
    ...
    try {
      Path sourceFilePath =
          Paths.get(
              ...
              getSourceFileName(fileSystemDropExporterConfig.getUploadDate(), bcp47tag));
  ...
  public void exportImportedFile(String filename, String fileContent, String comment)
    ...
    try {
      Path importedFilePath =
          Paths.get(
              fileSystemDropExporterConfig.getDropFolderPath(),
              DROP_FOLDER_IMPORTED_FILES_NAME,
              filename);
  ...
  public DropImporter getDropImporter() {
    ...
    Path localizedFolderPath =
        Paths.get(
            fileSystemDropExporterConfig.getDropFolderPath(), DROP_FOLDER_LOCALIZED_FILES_NAME);
  ...
  private void checkIsInitialized() {
    Preconditions.checkNotNull(
        fileSystemDropExporterConfig, getClass().getSimpleName() + " must be initialized first");
  }
}
