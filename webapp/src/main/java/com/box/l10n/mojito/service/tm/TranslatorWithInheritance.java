...
public class TranslatorWithInheritance {
  ...
  public TextUnitDTO getTextUnitDTO(String md5) {
    ...
    logger.debug(
        "Look for a textUnitDTO in target locale: {} for text unit with md5: {}",
    ...
    if (textUnitDTO != null) {
      ...
      logger.debug(
          "No textUnitDTO found for target locale, look for translations in parent locales");
  ...
  /**
   * Look for a translation in parent locale excluding the root locale for a given MD5.
   *
   * <p>The root locale (repository locale with parent locale null) is excluded for optimization
   * purpose, the translations fetched would be the same as the source, hence returning the source
  ...
  private TextUnitDTO getTextUnitDTOFromParents(String md5) {
    ...
    while (textUnitDTOWithInheritance == null
      ...
      logger.debug(
          ...
          repositoryLocaleForFind.getLocale().getBcp47Tag());
      TextUnitDTO textUnitDTO = getTextUnitDTO(md5, repositoryLocaleForFind.getLocale().getId());

      if (textUnitDTO != null) {
        logger.debug(
            "Found a TextUnitDTO using inheritance in locale: {}",
            repositoryLocaleForFind.getLocale().getBcp47Tag());
        textUnitDTOWithInheritance = textUnitDTO;
      } else {
        logger.debug(
            "No inherited TextUnitDTO in locale: {}",
            repositoryLocaleForFind.getLocale().getBcp47Tag());
      }
  ...
  /**
   * Gets a {@link TextUnitDTO} from the cache for a given locale and text unit MD5.
   *
   * @param md5 the MD5 of the text unit (see {@link
   *     TMService#computeTMTextUnitMD5(java.lang.String, java.lang.String, java.lang.String)})
   * @param localeId the {@link Locale#id}
   * @return a {@link TextUnitDTO} or {@code null} if no translation is available
   */
  private TextUnitDTO getTextUnitDTO(String md5, Long localeId) {
    Map<String, TextUnitDTO> translationsForLocaleMap =
        getTextUnitDTOsForLocaleMapFromCache(localeId);
    return translationsForLocaleMap.get(md5);
  }

  /**
   * Gets the cached map of {@link TextUnitDTO}s keyed by MD5 that contains all current translations
   * for a given locale.
   *
   * @param localeId the {@link Locale#id}
  ...
  private Map<String, TextUnitDTO> getTextUnitDTOsForLocaleMapFromCache(Long localeId) {
    ...
    Map<String, TextUnitDTO> translationsForLocaleMap =
        localeToTextUnitDTOsForLocaleMap.get(localeId);
  ...
  /**
   * Gets the map of {@link TextUnitDTO}s keyed by MD5 that contains all current translations for a
   * given locale.
   *
   * @param localeId the {@link Locale#id}
   * @return the map of {@link TextUnitDTO}s keyed by MD5 that contains all current translations for
   *     a given locale
   */
  private Map<String, TextUnitDTO> getTextUnitDTOsForLocaleByMD5(Long localeId) {
    return textUnitDTOsCacheService.getTextUnitDTOsForAssetAndLocaleByMD5(
        asset.getId(), localeId, statusFilter, false, UpdateType.ALWAYS);
  }
}
