...
public class TMTextUnitVariantCommentService {
  ...
  public void copyComments(Long sourceTmTextUnitVariantId, Long targetTMTextUnitVariantId) {
    ...
    logger.debug(
        "Copy TMTextUnitVariantComments from source TMTextUnitVariant: {} into target TMTextUnitVariant: {}",
        sourceTmTextUnitVariantId,
        targetTMTextUnitVariantId);

    for (TMTextUnitVariantComment comment :
  ...
  /**
   * Adds variant comments associated to the given TextUnitDTOs. It processes the TextUnitDTOs by
   * batch (500 at a time) to reduce the number of queries needed.
   *
   * @param textUnitDTOs List of the DTOs to enrich
   * @return List of DTOs containing their associated comments
   */
  public List<TextUnitDTOWithComments> enrichTextUnitDTOsWithComments(
    ...
    for (TextUnitDTO textUnitDTO : textUnitDTOs) {
      ...
      if (count % BATCH_SIZE == 0) {
        processTextUnitDTOsBatch(
            textUnitDTOsWithComments, textUnitDTOsForBatch, tmTextUnitVariantIdsForBatch);
   ...
   * @param tmTextUnitVariantIdsForBatch The list of {@link TMTextUnitVariant#id} to get comments
   *     for
   */
  private void processTextUnitDTOsBatch(
      List<TextUnitDTOWithComments> textUnitDTOsWithComments,
      List<TextUnitDTO> textUnitDTOsForBatch,
      List<Long> tmTextUnitVariantIdsForBatch) {

    Multimap<Long, TMTextUnitVariantComment> textUnitVariantCommentsMap =
        ArrayListMultimap.create();
    List<TMTextUnitVariantComment> textUnitVariantComments = new ArrayList<>();

    if (!tmTextUnitVariantIdsForBatch.isEmpty()) {
      textUnitVariantComments =
          tmTextUnitVariantCommentRepository.findByTmTextUnitVariantIdIn(
              tmTextUnitVariantIdsForBatch);
    }
    ...
    mergeCommentsIntoTextUnitDTOs(
        textUnitDTOsWithComments, textUnitDTOsForBatch, textUnitVariantCommentsMap);
  }
   ...
   * @param textUnitDTOsWithComments The list to be updated
   * @param textUnitDTOs List of the DTOs to enrich
   * @param textUnitVariantCommentsMap Map of {@link TMTextUnitVariant#id} => {@link
   *     TMTextUnitVariantComment} associated to these DTOs
   * @return The list of TextUnitDTOs containing their associated comments
   */
  private void mergeCommentsIntoTextUnitDTOs(
      List<TextUnitDTOWithComments> textUnitDTOsWithComments,
      List<TextUnitDTO> textUnitDTOs,
      Multimap<Long, TMTextUnitVariantComment> textUnitVariantCommentsMap) {
    ...
    for (TextUnitDTO textUnitDTO : textUnitDTOs) {
      ...
      Long tmTextUnitVariantId = textUnitDTO.getTmTextUnitVariantId();
      Collection<TMTextUnitVariantComment> variantComments =
          textUnitVariantCommentsMap.get(tmTextUnitVariantId);
