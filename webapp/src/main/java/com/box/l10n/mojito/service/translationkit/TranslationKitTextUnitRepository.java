...
public interface TranslationKitTextUnitRepository
    extends JpaRepository<TranslationKitTextUnit, Long> {

  TranslationKitTextUnit findByTranslationKitAndTmTextUnitAndTranslationKit_Locale(
      TranslationKit translationKit, TMTextUnit tmTextUnit, Locale locale);
  ...
  @Query(
      "select count(*) from #{#entityName} tktu where tktu.translationKit = ?1 and tktu.detectedLanguage != tktu.detectedLanguageExpected")
