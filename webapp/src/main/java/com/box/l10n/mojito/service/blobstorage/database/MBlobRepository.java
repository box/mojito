...
public interface MBlobRepository extends JpaRepository<MBlob, Long>, JpaSpecificationExecutor<MBlob> {
    ...
    @Query(value = "select mb.id from #{#entityName} mb where (unix_timestamp(mb.createdDate) + mb.expireAfterSeconds) < unix_timestamp()")
    List<Long> findExpired(PageRequest pageable);
