...
public class CommitSpecification {
    ...
    public static SingleParamSpecification<Commit> repositoryIdEquals(final Long repositoryId) {
        return new SingleParamSpecification<Commit>(repositoryId) {
            public Predicate toPredicate(Root<Commit> root, CriteriaQuery<?> query,
    ...
    public static SingleParamSpecification<Commit> commitNamesIn(final List<String> commitNames) {
        return new SingleParamSpecification<Commit>(commitNames) {
            @Override
            public Predicate toPredicate(Root<Commit> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
    ...
    public static SingleParamSpecification<Commit> pushRunNameEquals(final String pushRunName) {
        return new SingleParamSpecification<Commit>(pushRunName) {
            ...
            public Predicate toPredicate(Root<Commit> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
                ...
                Join<Commit, CommitToPushRun> commitToPushRunJoin = root.join(Commit_.commitToPushRun, JoinType.LEFT);
                Join<CommitToPushRun, PushRun> pushRunJoin = commitToPushRunJoin.join(CommitToPushRun_.pushRun,
                                                                                      JoinType.LEFT);

                Predicate conjunction = builder.conjunction();
                conjunction.getExpressions().add(builder.equal(pushRunJoin.get(PushRun_.name), pushRunName));
    ...
    public static SingleParamSpecification<Commit> pullRunNameEquals(final String pullRunName) {
        return new SingleParamSpecification<Commit>(pullRunName) {
            @Override
            public Predicate toPredicate(Root<Commit> root,
                                         CriteriaQuery<?> query,
                                         CriteriaBuilder builder) {

                Join<Commit, CommitToPullRun> commitToPullRunJoin = root.join(Commit_.commitToPullRun, JoinType.LEFT);
                Join<CommitToPullRun, PullRun> pullRunJoin = commitToPullRunJoin.join(CommitToPullRun_.pullRun,
                                                                                      JoinType.LEFT);

                Predicate conjunction = builder.conjunction();
                conjunction.getExpressions().add(builder.equal(pullRunJoin.get(PullRun_.name), pullRunName));
