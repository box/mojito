...
/**
 * This class is the meant to access TextUnitDTOs in an optmized way, reading most of data
...
public class TextUnitDTOsCacheService {
    ...
    @Timed
    public ImmutableMap<String, TextUnitDTO> getTextUnitDTOsForAssetAndLocaleByMD5(Long assetId, Long localeId, StatusFilter statusFilter, boolean isRootLocale, boolean updateBlob) {
        ImmutableList<TextUnitDTO> textUnitDTOsForAssetAndLocale = getTextUnitDTOsForAssetAndLocale(assetId, localeId, isRootLocale, updateBlob);
        ImmutableMap<String, TextUnitDTO> filteredWithStatus = filterWithStatus(statusFilter, textUnitDTOsForAssetAndLocale);
        return filteredWithStatus;
    }

    ImmutableMap<String, TextUnitDTO> filterWithStatus(StatusFilter statusFilter, ImmutableList<TextUnitDTO> textUnitDTOsForAssetAndLocale) {
    ...
    ImmutableList<TextUnitDTO> getTextUnitDTOsForAssetAndLocale(Long assetId, Long localeId, boolean isRootLocale, boolean updateBlob) {

        ImmutableList<TextUnitDTO> textUnitDTOs = textUnitDTOsCacheBlobStorage.getTextUnitDTOs(assetId, localeId);
    ...
    ImmutableList<TextUnitDTO> updateTextUnitDTOsWithDeltaFromDatabase(ImmutableList<TextUnitDTO> toUpdate, Long assetId, Long localeId, boolean isRootLocale) {
        ...
        // TODO(perf)(later) For certain use case like synching keep fetching that list will be impacting perf and potentially
        // consistancy so it might be preferable to read once upstream and pass it down
        //
        // reading this from the DB will become expensive eventually so it will have to be read from the asset cache
        // most likely. But it makes both system depends on each other...
        List<TMTextUnitCurrentVariantDTO> currentTranslations = getCurrentTranslationsOfAllTextUnits(assetId, localeId);
        List<Long> idsOfAllTextUnits = getIdsOfAllTextUnits(assetId);
        Set<Long> idsOfUsedTextUnits = getIdsOfUsedTextUnits(asset);
    ...
    Asset getAssetById(Long assetId) {
        return assetRepository.findById(assetId).orElseThrow(() -> new IllegalArgumentException("Asset missing for given id"));
    }
     ...
     */
    @Timed
    List<TMTextUnitCurrentVariantDTO> getCurrentTranslationsOfAllTextUnits(Long assetId, Long localeId) {
