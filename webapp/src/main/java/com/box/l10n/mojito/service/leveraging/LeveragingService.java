...
public class LeveragingService {
   ...
   *
   * <p>This process is about trying to identify renamed keys, comment changes, source string
   * modification. When such a change happens the system will copy over all the {@link
   * TMTextUnitVariant}s from the old {@link TMTextUnit} into the new one.
   *
   * <p>This code is not trying to find matches based on the target locale, it's really about trying
   * to find partial matches of the text units following maintenance operation (renamed key, minor
   * change to source string, etc).
   *
   * <p>Changes might be minor to fix typo or could require a review. Right now only a change in
   * content will require a review. Modification to a comment or renaming a key won't trigger a
   * review.
   *
   * <p>Note that if {@link TMTextUnitVariant} are added via TK import or workbench on the old
   * {@link TMTextUnit} the new mapped {@link TMTextUnit} will be missing the new translation. That
   * case will be handled later with regular "target" leveraging. This case should be rare and on
   * minimal dataset.
   *
   * @param tmTextUnits list of {@link TMTextUnit}s that needs to be processed. {@link TMTextUnit}s
   *     for which leveraged translations were found are removed from the list to prevent further
   *     processing.
   */
  ...
  /**
   * This will copy all translations from the source repository into the target repository (or
   * asset), overriding any translation already existing.
   *
   * <p>2 match modes are available:
   *
   * <p>1) Matches are performed based on MD5, if the repository has multiple text units with same
   * MD5 the source will be arbitrarily chosen.
   *
   * <p>2) Matches are performed based on content only (exact match), if the repository has multiple
   * text units with same content it will first check for string with same IDs and then the source
  ...
  void copyTmBetweenRepositories(CopyTmConfig copyTmConfig)
      throws RepositoryWithIdNotFoundException, AssetWithIdNotFoundException {

    logger.debug(
        "Copy TM, source repository: {}, source asset: {}, target repository: {}, target asset: {}",
    ...
    Repository sourceRepository =
        getRepositoryForCopy(copyTmConfig.getSourceRepositoryId(), copyTmConfig.getSourceAssetId());
    Repository targetRepository =
        getRepositoryForCopy(copyTmConfig.getTargetRepositoryId(), copyTmConfig.getTargetAssetId());
    ...
    CopyTmConfig.TargetStatusFilter targetStatusFilter = copyTmConfig.getTargetStatusFilter();
  ...
  void copyTranslationBetweenTextUnits(Map<Long, Long> sourceToTargetTmTextUnitId) {
    if (sourceToTargetTmTextUnitId != null) {
      for (Map.Entry<Long, Long> sourceToTargetTmTextUnit : sourceToTargetTmTextUnitId.entrySet()) {
        copyTranslationsBetweenTextUnits(
            sourceToTargetTmTextUnit.getKey(), sourceToTargetTmTextUnit.getValue());
      }
    } else {
  ...
  Repository getRepositoryForCopy(Long repositoryId, Long assetId)
      throws AssetWithIdNotFoundException, RepositoryWithIdNotFoundException {
