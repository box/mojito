...
public class SmartlingJsonConverter {
        ...
        textUnitDTOs.stream().forEach(t ->
                strings.add(
                        objectMapper.createObjectNode()
                                .put("key", smartlingJsonKeys.toKey(t.getTmTextUnitId(), t.getName(), t.getAssetPath()))
    ...
    public ImmutableList<TextUnitDTO> jsonStringToTextUnitDTOs(String localizedFileContent, BiConsumer<TextUnitDTO, String> stringConsumer) {
        ImmutableList<TextUnitDTO> textUnitDTOs = Streams.stream(objectMapper.readTreeUnchecked(localizedFileContent).get("strings").iterator())
                .map(node -> {
