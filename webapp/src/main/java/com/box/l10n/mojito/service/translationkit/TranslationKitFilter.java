...
public class TranslationKitFilter implements IFilter {
  ...
  public void open(RawDocument input, boolean generateSkeleton) {
    ...
    logger.debug(
        "Open raw document: get the textunitDTOs for translationKitId: {}", translationKitId);
    ...
    if (useInheritance) {
      textUnitDTOsForTranslationKit =
          translationKitService.getTextUnitDTOsForTranslationKitWithInheritance(translationKitId);
    } else {
      textUnitDTOsForTranslationKit =
          translationKitService.getTextUnitDTOsForTranslationKit(translationKitId, type);
    }

    textUnitsIterator = textUnitDTOsForTranslationKit.iterator();

    StartDocument startDoc =
        new StartDocument("Translation Kit translationKitId: " + translationKitId);
  ...
  public Event next() {
    ...
    // TODO(P1) refactor? have a stack initialized (getTextUnitDTOsForTranslationKit) in the ctor
    ...
    if (textUnitDTO.getTarget() != null) {
      ...
      state =
          TranslationKit.Type.TRANSLATION.equals(type)
              ? XliffState.NEEDS_TRANSLATION
              : XliffState.NEEDS_REVIEW_TRANSLATION;
      ...
      // TODO(P1):new status Support more state later, for now just re-implement with enum instead
      // of boolean
      importNoteBuilder.setNeedsReview(
          TMTextUnitVariant.Status.REVIEW_NEEDED.equals(textUnitDTO.getStatus()));

      if (!Strings.isNullOrEmpty(textUnitDTO.getTargetComment())) {
        textUnit.setProperty(
            new Property(
                com.box.l10n.mojito.okapi.Property.TARGET_COMMENT, textUnitDTO.getTargetComment()));
      }

      textUnit.setProperty(
          new Property(
              com.box.l10n.mojito.okapi.Property.TARGET_NOTE, importNoteBuilder.toString()));
    } else {
