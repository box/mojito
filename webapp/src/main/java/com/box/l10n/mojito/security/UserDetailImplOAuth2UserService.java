...
/**
 * Mostly a copy of {@link DefaultOAuth2UserService} that returns a {@link UserDetailsImpl} as
 * principal.
 *
 * <p>Also it adds an option to unwrap the user attributes if {@link
 * SecurityConfig.OAuth2#unwrapUserAttributes} is provided. This allows to process payloads similar
 * to
 *
 * <p>*
 *
 ...
 *  * </pre>
 */
public class UserDetailImplOAuth2UserService
  ...
  OAuth2User getOAuth2UserDetailsImpl(
      OAuth2UserRequest userRequest,
      String userNameAttributeName,
      Map<String, Object> userAttributes) {

    logger.debug(
        "Get OAuth2UserDetailsImpl user for client registration: {}",
        userRequest.getClientRegistration().getRegistrationId());

    SecurityConfig.OAuth2 oAuth2ConfigForRegistration =
        securityConfig
            .getoAuth2()
            .getOrDefault(
                userRequest.getClientRegistration().getRegistrationId(),
    ...
    String givenName =
        (String) userAttributes.get(oAuth2ConfigForRegistration.getGivenNameAttribute());
    String surname = (String) userAttributes.get(oAuth2ConfigForRegistration.getSurnameAttribute());
    String commonName =
        (String) userAttributes.get(oAuth2ConfigForRegistration.getCommonNameAttribute());
  ...
  String getUsername(
    ...
    if (usernameObject == null) {
      throw new IllegalArgumentException(
          "Missing required user name (attribute: "
  ...
  Map<String, Object> optionallyUnwrapUserAttributes(
      Map<String, Object> userAttributes, SecurityConfig.OAuth2 oAuth2ConfigForRegistration) {
    if (oAuth2ConfigForRegistration.getUnwrapUserAttributes() != null) {
      logger.debug(
          "Unwrap user attributes using key: {}",
          oAuth2ConfigForRegistration.getUnwrapUserAttributes());
      ...
      if (!userAttributes.containsKey(unwrapUserAttributes)) {
        throw new IllegalArgumentException(
            "Missing attribute '"
                + unwrapUserAttributes
                + "' in attributes for unwrapping user attributes");
      }
