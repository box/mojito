...
public class TranslationMerger {
  ...
  /**
   * Merges two sets of translations into a new object. Whenever multiple translations are found for
   * the same target bcp47tag, only one translation will be picked based on the priority of the
   * source of the translation.
   */
  public ImmutableMap<String, ImmutableList<TranslationDTO>> getMergedTranslationsBySourceText(
      ImmutableMap<String, ImmutableList<TranslationDTO>> translationsBySourceTextA,
      ImmutableMap<String, ImmutableList<TranslationDTO>> translationsBySourceTextB) {
    ImmutableList<String> uniqueTextSources =
        getUniqueKeys(translationsBySourceTextA, translationsBySourceTextB);
    ...
    return uniqueTextSources.stream()
        .map(
            sourceText -> {
              ...
              ImmutableList<String> uniqueBcp57Tags =
                  getUniqueKeys(translationsByBcp47TagA, translationsByBcp47TagB);

              ImmutableList<TranslationDTO> translations =
                  uniqueBcp57Tags.stream()
                      .map(
                          bcp47Tag ->
   ...
   * where the lower the number defined in {@link TranslationSource} is the one that will be
   * prioritized.
   *
   * <p>If either of the two translation objects is null, the non-null one will be returned.
   */
  public TranslationDTO getPriorityTranslation(
      TranslationDTO translationA, TranslationDTO translationB) {
    ...
    if (translationA.getTranslationSource().getPriority()
        < translationB.getTranslationSource().getPriority()) {
      return translationA;
    } else {
      return translationB;
    }
  ...
  private ImmutableList<String> getUniqueKeys(Map<String, ?> map1, Map<String, ?> map2) {
