  ...
  public static byte[] decompressOrOriginal(byte[] bytes) {
    ...
    if (hasGZipMagicNumber(bytes)) {
      try {
        ...
        logger.debug(
            "Not able to decompress, return original. Allows to accept content with GZip header that "
                + "is not actual GZip content. Drawback is that it could hide other IOException",
            e);
      }
    }
