...
/**
 * Implementation of {@link PollableFuture} to be used in function instrumented
 * with {@link Pollable} to return the result.
 *
 * @author jaurambault
 * @param <T> result type
 */
public class PollableFutureTaskResult<T> implements PollableFuture<T> {
    ...
    /**
     * if not {@code null}, the new {@link PollableTask#message} to be saved by
     * the aspect
     */
    String messageOverride;

    /**
     * if not {@code null}, the new {@link PollableTask#expectedSubTaskNumber}
     * to be saved by the aspect
     */
