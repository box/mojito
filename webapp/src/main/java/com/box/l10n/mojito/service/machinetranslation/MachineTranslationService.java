...
public class MachineTranslationService {
  ...
  public TranslationsResponseDTO getTranslations(
    ...
    if (!skipLeveraging) {
      ...
      // Only MT strings for which we didn't leverage translationsBySourceText for all the expected
      // target languages.
      // Note: this would benefit from further optimization at some point to take into account
      // partially leveraged
      // text units - though that would require additional MT API calls for certain MT engine
      // implementations.
      if (!leveragedTranslationsBySourceText.isEmpty()) {
        untranslatedStrings =
            textSources.stream()
                .filter(
                    sourceText ->
                        !leveragedTranslationsBySourceText.containsKey(sourceText)
                            || leveragedTranslationsBySourceText.get(sourceText).size()
                                != targetBcp47Tags.size())
                .collect(Collectors.toList());
      }
    }

    ImmutableMap<String, ImmutableList<TranslationDTO>> machineTranslationsBySourceText =
    ...
    translationsBySourceText =
        translationMerger.getMergedTranslationsBySourceText(
            translationsBySourceText, machineTranslationsBySourceText);
  ...
  ImmutableMap<String, ImmutableList<TranslationDTO>> getMachineTranslationBySourceText(
    ...
    if (textSources.isEmpty()) {
      ...
      result =
          machineTranslationEngine.getTranslationsBySourceText(
              textSources,
              sourceBcp47Tag,
              targetBcp47Tags,
              TextType.HTML,
  ...
  ImmutableMap<String, ImmutableList<TranslationDTO>> getLeveragedTranslationsBySourceText(
    ...
    return textSources.stream()
        .map(
            (sourceText) -> {
              // Note: to further increase leverage performance at a later date, we should introduce
              // a
              // bulk leveraging function that will take in a list of source strings, a list of
              // target
              // locales and will return one single result for each source string and locale
              // combination.
              ...
              // Note: the leveraging mode is locale code specific. If the translation requests ask
              // for "pt" but
              // we only have a "pt-BR" translation, or the other way around, we do not use the
              // leveraged match.
              ImmutableList<TranslationDTO> translations =
                  targetBcp47Tags.stream()
                      .filter(mostRecentTranslationsByLanguage::containsKey)
                      .map(
                          targetBcp47Tag -> {
                            TextUnitDTO matchedLanguageTextUnit =
                                mostRecentTranslationsByLanguage.get(targetBcp47Tag);
                            ...
                            translation.setMatchedTextUnitId(
                                matchedLanguageTextUnit.getTmTextUnitId());
                            translation.setMatchedTextUnitVariantId(
                                matchedLanguageTextUnit.getTmTextUnitVariantId());
  ...
  private ImmutableMap<String, TextUnitDTO> getMostRecentTranslationsByLanguage(
      List<Long> repositoryIds, List<String> repositoryNames, String sourceText) {
    List<TextUnitDTO> textUnitDTOsForLeveraging =
        getLeveragedTextUnitDTOS(sourceText, repositoryIds, repositoryNames);
  ...
  List<TextUnitDTO> getLeveragedTextUnitDTOS(
    ...
    return leveragerByContentAndRepository.getLeveragingMatches(searchTmTextUnit, null, null);
  }
  ...
  /**
   * Returns a translation response with the data sorted according to the order in the provided text
   * sources and target Bcp47 tags.
   */
  private TranslationsResponseDTO getSortedTranslationsResponse(
      List<String> textSources,
      List<String> targetBcp47Tags,
      ImmutableMap<String, ImmutableList<TranslationDTO>> translationsBySourceText) {
    ImmutableList<TextUnitTranslationGroupDTO> textUnitTranslationGroups =
        textSources.stream()
            .map(
                sourceText -> {
                  ...
                  ImmutableList<TranslationDTO> sortedTranslations =
                      targetBcp47Tags.stream()
                          .map(
                              targetBcp47Tag ->
                                  getTranslationOrDefaultUntranslated(
                                      sourceText, unorderedTranslationsByBcp47Tag, targetBcp47Tag))
                          .collect(ImmutableList.toImmutableList());

                  TextUnitTranslationGroupDTO textUnitTranslationGroup =
                      new TextUnitTranslationGroupDTO();
  ...
  private TranslationDTO getTranslationOrDefaultUntranslated(
      String sourceText,
      ImmutableMap<String, TranslationDTO> translationsByBcp47Tag,
  ...
  private ImmutableMap<String, TranslationDTO> getTranslationsByBcp47Tag(
      ImmutableMap<String, ImmutableList<TranslationDTO>> translationsBySourceText,
      String sourceText) {
