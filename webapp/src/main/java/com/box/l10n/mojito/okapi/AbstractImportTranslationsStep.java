 ...
 *
 * <p>Extending class will define the logic to perform the lookup of the TMTextUnits in which
...
public abstract class AbstractImportTranslationsStep extends AbstractMd5ComputationStep {
  ...
  /**
   * Optional parameter that can be used to force the import of new translation with a given status.
   * If not used it will use the state form the XLIFF.
   */
  TMTextUnitVariant.Status importWithStatus = null;

  /**
   * Keep track of Text units that are in the XLIFF but not in the database (file corruption).
   *
   * <p>Use String and not Long because the XLIFF
   */
  Set<String> notFoundTextUnitIds;

  /**
   * Indicates if the whole document should be review because there was an issue importing one of
   * the text unit.
   */
  ...
  /**
   * TODO reusing filter options in a step might be debatable.. but it is handy to pass the target
   * comment without a big change. Doing that for now
   */
  ...
  /**
   * This step can be used to import translation from monolingual and bi-lingual document. The text
   * container that contains the translation to be imported is either the source or target container
   *
  ...
  /**
   * Indicates if the translation should be imported or not and with what status.
   *
   * <p>If null is returned then the import should be skipped.
   *
   * <p>This is allow to define import strategies when the source and target are the same, etc.
   *
  ...
  protected Event handleTextUnit(Event event) {
    ...
    if (textUnit.isTranslatable()) {
      ...
      if (tmTextUnit == null) {
        ...
        logger.debug(
            "Missing target container for TMTextUnit with id: {}. Skipping it...",
            textUnit.getId());
        importNoteBuilder = new ImportNoteBuilder();
        importNoteBuilder.setMustReview(true);
        importNoteBuilder.addError(
            "Target missing for text unit with id: " + textUnit.getId() + ", Skipping it...");
        documentReviewNeeded = true;
      } else {
        ...
        if (statusForImport != null) {
          TMTextUnitVariant importTextUnit =
              importTextUnit(tmTextUnit, target, statusForImport, createdDate);
  ...
  void setStateProperty(TextContainer target, XliffState xliffState) {
    if (target != null) {
      target.setProperty(
          new Property(com.box.l10n.mojito.okapi.Property.STATE, xliffState.toString()));
    }
  }

  /**
   * Gets the "state" based on {@link TMTextUnitVariant#isIncludedInLocalizedFile() } and {@link
   * TMTextUnitVariant#getStatus() }.
   *
   * @param tmTextUnitVariant contains information to decide if the target should be reviewed or not
   *     return the xliff "state"
   */
  ...
  /**
   * Creates and populate {@link ImportNoteBuilder} from a {@link TMTextUnitVariant}.
   *
   * @param tmTextUnitVariant contains messages that need to be appended to the {@link
   *     ImportNoteBuilder} and set the review status.
   * @return the {@link ImportNoteBuilder} containing message from the {@link TMTextUnitVariant}
   */
  private ImportNoteBuilder buildImportNoteBuilderFromVariant(TMTextUnitVariant tmTextUnitVariant) {
    ...
    // TODO:new status Support more state later, for now just re-implement with enum instead of
    // boolean
  ...
  /**
   * Gets the status to be used for adding a translation given a target and its state.
   *
   * <p>If no supported state is present then default to "review needed"
   *
  ...
  TMTextUnitVariant importTextUnit(
    ...
    logger.debug(
        "Looking at TMTextUnitVariantCommentAnnotations to decide if translation should be included or needs review (by default included and no review)");
    ...
    if (tmTextUnitVariantCommentAnnotations.hasCommentWithErrorSeverity()) {
      logger.debug(
          "translation has comments with error, document review is needed and translation is excluded from localized file");
    ...
    logger.debug(
        "Adding TMTextUnitVariant for textUnitId: {}, localeId: {}, status: {}, target: {}",
        tmTextUnitId,
        targetLocaleId,
        status,
        targetString);
    ...
    for (TMTextUnitVariantCommentAnnotation tmTextUnitVariantCommentAnnotation :
        tmTextUnitVariantCommentAnnotations.getAnnotations()) {
  ...
  protected Event handleEndDocument(Event event) {
    ...
    ImportTranslationsStepAnnotation importTranslationsStepAnnotation =
        new ImportTranslationsStepAnnotation();
    rawDocument.setAnnotation(importTranslationsStepAnnotation);

    if (documentReviewNeeded) {
      importTranslationsStepAnnotation.setComment(
          "The document needs review, see import details in the file");
    }
  ...
  /**
   * Gets a {@link TMTextUnit} that matches the information contains in the {@link ITextUnit} to add
  ...
  /**
   * Indicates if the translation to be added should be added as as current translation or just as
