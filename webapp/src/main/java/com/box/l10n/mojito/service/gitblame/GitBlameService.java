...
public class GitBlameService {
  ...
  @Autowired GitBlameRepository gitBlameRepository;
   ...
   *
   * <p>Use the {@link TextUnitSearcher#search(TextUnitSearcherParameters)} to get the text units
   * for which information is required.
   *
   * <p>For each of text unit DTO that the searcher returns, get the usage and the git blame
   * information to build the payload returned.
   *
   * <p>Usually this method should be called for the root locale since we're looking for information
   * on the source ({@link com.box.l10n.mojito.entity.TMTextUnit} but the search parameters can
   * actuall be anything.
   *
  ...
  public List<GitBlameWithUsage> getGitBlameWithUsages(
    ...
    List<GitBlameWithUsage> gitBlameWithUsages =
        convertTextUnitsDTOsToGitBlameWithUsages(textUnitDTOS);
  ...
  Set<Long> getTmTextUnitIds(List<GitBlameWithUsage> gitBlameWithUsages) {
    return gitBlameWithUsages.stream()
        .map(GitBlameWithUsage::getTmTextUnitId)
  ...
  void enrichTextUnitsWithGitBlame(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    Map<Long, GitBlame> currentGitBlameForTmTextUnitIds =
        getCurrentGitBlameForTmTextUnitIds(tmTextUnitIds);

    for (GitBlameWithUsage gitBlameWithUsage : gitBlameWithUsages) {
      gitBlameWithUsage.setGitBlame(
          currentGitBlameForTmTextUnitIds.get(gitBlameWithUsage.getTmTextUnitId()));
    }
  ...
  void enrichTextUnitsWithUsages(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    for (GitBlameWithUsage textUnitWithUsage : gitBlameWithUsages) {
      if (textUnitWithUsage.getAssetTextUnitId() != null) {
        assetTextUnitIdToGitBlameWithUsage.put(
            textUnitWithUsage.getAssetTextUnitId(), textUnitWithUsage);
      }
    }
  ...
  void enrichTextUnitsWithScreenshots(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    Map<Long, Set<Screenshot>> screenshotsByTextUnit =
        screenshotService.getScreenshotsByTmTextUnitId(tmTextUnitIds);

    gitBlameWithUsages.forEach(
        gitBlameWithUsage -> {
          Set<Screenshot> screenshots =
              screenshotsByTextUnit.get(gitBlameWithUsage.getTmTextUnitId());
  ...
  void enrichTextUnitsWithThirdPartyTextUnitId(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    List<ThirdPartyTextUnit> thirdPartyTextUnitList =
        thirdPartyTextUnitRepository.findByTmTextUnitIdIn(
            gitBlameWithUsagesByTmTextUnitId.keySet());

    for (ThirdPartyTextUnit thirdPartyTextUnit : thirdPartyTextUnitList) {
      GitBlameWithUsage gitBlameWithUsage =
          gitBlameWithUsagesByTmTextUnitId.get(thirdPartyTextUnit.getTmTextUnit().getId());
  ...
  void enrichTextUnitsWithAssetVirtualTag(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    gitBlameWithUsages.forEach(
        gitBlameWithUsage -> {
          gitBlameWithUsage.setVirtual(virtualAssetIds.contains(gitBlameWithUsage.getAssetId()));
        });
  }

  private Set<Long> getAssetIds(List<GitBlameWithUsage> gitBlameWithUsages) {
    return gitBlameWithUsages.stream()
        .map(GitBlameWithUsage::getAssetId)
        .collect(Collectors.toSet());
  }

  /**
   * Save the GitBlame information for a list of {@link com.box.l10n.mojito.entity.TMTextUnit}. The
   * text units are identified by the {@link GitBlame#getId()} and the git blame info come for the
   * {@link GitBlameWithUsage#getGitBlame()} method.
   *
  ...
  public PollableFuture<Void> saveGitBlameWithUsages(List<GitBlameWithUsage> gitBlameWithUsages) {
    ...
    Map<Long, GitBlame> currentGitBlameForTmTextUnitIds =
        getCurrentGitBlameForTmTextUnitIds(gitBlameWithUsagesByTmTextUnitId.keySet());

    for (Map.Entry<Long, GitBlameWithUsage> gitBlameWithUsageEntry :
        gitBlameWithUsagesByTmTextUnitId.entrySet()) {
  ...
  /**
   * Builds a map from {@link com.box.l10n.mojito.entity.TMTextUnit#id} id to {@link
   * GitBlameWithUsage}, removing duplicated entries for a given {@link
   * com.box.l10n.mojito.entity.TMTextUnit#id}. The first entry in the list will be used to get the
   * git information. This is useful in case where there are multiple entries for different plural
   * forms. The client may end up sending the same information for each form but we just need to
   * save it once since it is the same entity. We take arbitrarly the first entity.
   *
   * @param gitBlameWithUsages
   * @return
   */
  HashMap<Long, GitBlameWithUsage> getGitBlameWithUsagesByTmTextUnitId(
      List<GitBlameWithUsage> gitBlameWithUsages) {
