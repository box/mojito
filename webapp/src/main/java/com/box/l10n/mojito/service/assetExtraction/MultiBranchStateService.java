...
public class MultiBranchStateService {
  ...
  public MultiBranchStateService(
    ...
    this.assetTextUnitToTMTextUnitRepository =
        Preconditions.checkNotNull(assetTextUnitToTMTextUnitRepository);
  ...
  MultiBranchState getAndSaveInitialMultiBranchStateFromDatabase(
    ...
    MultiBranchState multiBranchStateForAssetExtractionId =
        getInitialMultiBranchStateFromDatabase(assetExtractionId, version);
    logger.debug("Save the initial state for asset extraction id: {}", assetExtractionId);
    multiBranchStateBlobStorage.putMultiBranchStateForAssetExtractionId(
        multiBranchStateForAssetExtractionId, assetExtractionId, 0);
   ...
   *
   * <p>This is needed to migrate from the old extraction logic to the new one.
   *
   * <p>When working on new repositories (or assets) the data returned by the database will be
   * empty, and the MultiBranchState will also be empty, so it should have minor impact later on the
   * new implementation. In other words, if not a migrated repository, this code is not needed.
   *
            ...
            .map(
                assetTextUnit -> {
                  ...
                  builder
                      .tmTextUnitId(
                          assetTextUnitToTMTextUnitRepository
                              .findTmTextUnitId(assetExtractionId, assetTextUnit.getId())
                              .orElse(null))
                  ...
                  if (assetTextUnit.getPluralForm() != null) {
                    builder
                        .pluralForm(assetTextUnit.getPluralForm().getName())
