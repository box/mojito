...
public class BoxSDKService {
     ...
     * @throws BoxSDKServiceException When an error occurred while retrieving
     * the folder
     */
     ...
     * @throws BoxSDKServiceException When an error occurred while creating the
     * folder
     */
     ...
     *
     * @param folderName The name of the shared folder to create
     * @param parentId The ID of the parent folder where the folder should be
     * created
     * @return The created shared folder
     * @throws BoxSDKServiceException When an error occurred while creating the
     * folder
     */
    public BoxFolder createSharedFolder(String folderName, String parentId) throws BoxSDKServiceException {
     ...
     * <p>
     * Creates new file if needed or update existing file.
     *
     ...
     * @throws BoxSDKServiceException When an error occurred while uploading the
     * file
     */
    public BoxFile uploadFile(String folderId, String filename, String filecontent) throws BoxSDKServiceException {
        ...
        try {
            ...
            if (uploadFile == null) {
                ...
                BoxFile.Info uploadFileInfo = boxFolder.uploadFile(IOUtils.toInputStream(filecontent, StandardCharsets.UTF_8), filename);
     ...
     * @throws BoxSDKServiceException When an error occurred while listing the
     * files
     */
     ...
     * @throws BoxSDKServiceException When an error occurred while getting the
     * file content
     */
    ...
    /**
     * Returns the HTML code to create an Embed Widget pointing to the given
     * folder
     *
    ...
    public String getBoxEmbedWidgetIFrame(BoxFolder boxFolder) {
        ...
        return "<iframe src=\"" + url + "?view=list&sort=date&theme=blue\" width=\"500\" height=\"400\" show_parent_path=\"yes\" "
                + "frameborder=\"0\" allowfullscreen webkitallowfullscreen mozallowfullscreen oallowfullscreen msallowfullscreen></iframe>";
    }

    /**
     * Get a folder given the folder name from the shared (drop) folder, parent
     * folder. This only searches one level deep.
     *
    ...
    public BoxFolder getFolderWithName(String folderName) throws BoxSDKServiceException {
        return this.getFolderWithNameAndParentFolderId(folderName, getBoxSDKServiceConfig().getRootFolderId());
    }

    /**
     * Get a folder given the folder name from the parent folder. This only
     ...
     * the folder
     */
    public BoxFolder getFolderWithNameAndParentFolderId(String folderName, String parentFolderId) throws BoxSDKServiceException {
     ...
     * @throws BoxSDKServiceException When an error occurred while deleting the
     * folder
     */
     ...
     * @throws BoxSDKServiceException
     */
    public void deleteFolderContentOlderThan(String folderId, DateTime olderThan) throws BoxSDKServiceException {
        try {
            BoxFolder boxFolder = new BoxFolder(getBoxAPIConnection(), folderId);
            ...
            String msg = "Error trying to delete content older than: " + olderThan.toString() + " in folder: " + folderId;
            logger.error(msg, e);
            throw new BoxSDKServiceException(msg, e);
        }
    }
     ...
     * @throws BoxSDKServiceException When an error occurred while adding the
     * comment
     */
    ...
    /**
     * Indicates if an exception is due to trying to add a comment that is the
     * same as the last comment.
     *
     * @param e the exception that contains details about the error
     * @return {@code true} if the exception is due to trying to add a comment
     * that is the same as last comment else {@code false}
     */
    private boolean isSimilarCommentException(BoxAPIException e) {
        //TODO(P1) would be better to properly parse the response
        return e.getResponseCode() == HttpStatus.CONFLICT.value() && e.getResponse().contains("recent_similar_comment");
    }
}
