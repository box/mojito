...
/**
 * This removes asset extraction by branch that have an asset extraction that is used as last asset
 * extraction.
 *
 * <p>That happens for repository that a single branch in the old implementation. If not removed
 * then it leads to the asset being re-processed all the time and potentially if adding branches the
 * state would become corrupted.
 *
 * @author jaurambault
 */
...
public class AssetExtractionByBranchRemoverJob implements Job {
  ...
  @Bean
  SimpleTriggerFactoryBean triggerAssetExtractionByBranchRemover(
      @Qualifier("jobDetailAssetExtractionByBranchRemover") JobDetail job) {
