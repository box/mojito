...
public class ImportExportedXliffStep extends BasePipelineStep {
    ...
    private void initAssetForTargetXliff(String assetPath) {
        ...
        if (asset == null) {
            //TODO(P1) all the runtime should become typed and propagated to client layer properly
            String msg = "There is no asset found with this path, make sure to first import the source asset first";
            logger.debug(msg);
            throw new RuntimeException(msg);
        }
    }
    ...
    /**
     * Gets the map of MD5s to {@link TMTextUnit#id}s of the {@link TMTextUnit}s
     * related to an {@link Asset}.
     *
