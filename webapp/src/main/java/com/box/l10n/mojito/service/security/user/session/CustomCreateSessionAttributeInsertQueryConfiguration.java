...
/**
 * Workaround for issue <a
 * href="https://github.com/spring-projects/spring-session/issues/1213">Concurrent requests adding
 * the same session attribute result in duplicate key constraint violation</a> to change the Create
 * Session Attribute query to use an MySQL UPDATE command to update the ATTRIBUTE_BYTES of the
 * existing row if a duplicate key is encountered.
 *
 * <p><b>NOTE:</b>This custom insert query should only be used with a MySQL database.
 *
 * <p>Updated later for springboot 2.5 upgrade and actual change in spring session:
...
@ConditionalOnProperty(
    value = "l10n.spring.session.use-custom-mysql-create-session-attribute-query",
    havingValue = "true")
public class CustomCreateSessionAttributeInsertQueryConfiguration
    extends JdbcHttpSessionConfiguration {

  static Logger logger =
      LoggerFactory.getLogger(CustomCreateSessionAttributeInsertQueryConfiguration.class);
  ...
  private String getCustomCreateSessionAttributeQuery() {
    return StringUtils.replace(
        CREATE_SESSION_ATTRIBUTE_QUERY_ON_DUPLICATE_KEY_UPDATE, "%TABLE_NAME%", getTableName());
  }

  private String getTableName() {
    return customTableName != null
        ? customTableName
