...
public class PollableTaskBlobStorage {
    ...
    public <T> T getInput(Long pollableTaskId, Class<T> clazz) {
        String inputName = getInputName(pollableTaskId);
        String inputJson = structuredBlobStorage.getString(POLLABLE_TASK, inputName)
    ...
    public String getOutputJson(Long pollableTaskId) {
        String outputName = getOutputName(pollableTaskId);
        String outputJson = structuredBlobStorage.getString(POLLABLE_TASK, outputName)
                .orElseThrow(() -> new RuntimeException("Can't get the output json for: " + pollableTaskId));
        return outputJson;
    }
