    ...
    public static SingleParamSpecification<Asset> branchId(Long branchId, Boolean deleted) {
        return new SingleParamSpecification<Asset>(branchId) {
            ...
            public Predicate toPredicate(Root<Asset> root,
                ...
                SetJoin<Asset, AssetExtractionByBranch> assetAssetExtractionByBranchSetJoin = root.join(Asset_.assetExtractionByBranches, JoinType.LEFT);
                Join<AssetExtractionByBranch, Branch> assetExtractionByBranchBranchJoin = assetAssetExtractionByBranchSetJoin.join(AssetExtractionByBranch_.branch, JoinType.LEFT);

                Predicate conjunction = builder.conjunction();
                conjunction.getExpressions().add(builder.equal(assetExtractionByBranchBranchJoin.get(Branch_.id), branchId));

                if (deleted != null) {
                    conjunction.getExpressions().add(builder.equal(assetAssetExtractionByBranchSetJoin.get(AssetExtractionByBranch_.deleted), deleted));
                }
