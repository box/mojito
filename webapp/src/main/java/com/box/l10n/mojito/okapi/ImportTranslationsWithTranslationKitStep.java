...
public class ImportTranslationsWithTranslationKitStep extends ImportTranslationsByIdStep {
    ...
    void extractTranslationKit(Event event) throws ImportTranslationsStepException {
        ...
        try {
            ...
            logger.debug("Cannot convert sub document name: " + startSubDocument.getName() + " into Long (TranslationKit id)", nfe);
        }

        if (translationKit == null) {
            String msg = "No TranslationKit can be found for id: " + startSubDocument.getName() + " to perform this import";
            logger.debug(msg);
            throw new ImportTranslationsStepException(msg);
        }

        translationKitExportedAndCurrentTUVs = translationKitService.getTranslationKitExportedAndCurrentTUVs(translationKit.getId());
    }
    ...
    /**
     * Adds the translations as current translation if there wasn't any other
     * translation added between the time the translation kit was exported and
     * the import time. Else just save the translation as a variant, that can be
     * later access looking at history.
     */
    @Override
    @Transactional
    TMTextUnitVariant importTextUnit(TMTextUnit tmTextUnit, TextContainer target, TMTextUnitVariant.Status status, DateTime createdDate) {
        TMTextUnitVariant importTextUnit = super.importTextUnit(tmTextUnit, target, status, createdDate);
        translationKitService.markTranslationKitTextUnitAsImported(translationKit, importTextUnit);
        return importTextUnit;
    }

    /**
     * The text unit should be imported if the current translation is the same
     * as the previously imported translation in this TK (a re-import is being
     * performed) or if no translation has been added since the TK
     * was exported (this includes not having translation for this text unit).
     *
     * Note that deleting or changing the status of translation will prevent a 
     * reimport.
     * 
     * @param tmTextUnitId
     * @return
    ...
    boolean shouldImportAsCurrentTranslation(Long tmTextUnitId) {
        ...
        TranslationKitExportedImportedAndCurrentTUV translationKitExportedAndCurrentTUV = translationKitExportedAndCurrentTUVs.get(tmTextUnitId);

        boolean isTKReimport = Objects.equals(translationKitExportedAndCurrentTUV.getCurrentTmTextUnitVariant(), translationKitExportedAndCurrentTUV.getImportedTmTextUnitVariant());
        boolean noExternalTranslationAdded = Objects.equals(translationKitExportedAndCurrentTUV.getCurrentTmTextUnitVariant(), translationKitExportedAndCurrentTUV.getExportedTmTextUnitVariant());
