...
public class BoxDropExporter implements DropExporter {
  ...
  public void setConfig(String config)
      throws AlreadyInitializedExporterException, DropExporterException {
  ...
  @Override
  public void init(String dropGroupName, String dropName)
    ...
    if (boxDropExporterConfig != null) {
      logger.debug(
          "boxDropExporterConfig was set, the filter is already initialized and ready to be used");
    } else {
  ...
  /**
   * Creates the drop folder tree that will be used to upload files and sets the {@link
   * #boxDropExporterConfig} accordingly.
   *
   ...
   */
  private void createDropFolderTree(String dropGroupName, String dropName, Date uploadDate)
    ...
    BoxFolder dropFolder =
        boxSDKService.createSharedFolder(convertToBoxItemName(dropName), repositoryFolder.getID());
    String dropFolderId = dropFolder.getID();

    BoxFolder sourceFilesFolder =
        boxSDKService.createSharedFolder(DROP_FOLDER_SOURCE_FILES_NAME, dropFolderId);
    BoxFolder localizedFilesFolder =
        boxSDKService.createSharedFolder(DROP_FOLDER_LOCALIZED_FILES_NAME, dropFolderId);
    BoxFolder importedFilesFolder =
        boxSDKService.createSharedFolder(DROP_FOLDER_IMPORTED_FILES_NAME, dropFolderId);
    BoxFolder queriesFolder =
        boxSDKService.createSharedFolder(DROP_FOLDER_QUERIES_NAME, dropFolderId);
    BoxFolder quotesFolder =
        boxSDKService.createSharedFolder(DROP_FOLDER_QUOTES_NAME, dropFolderId);
  ...
  /**
   * Gets the drop group folder that contains the drops. If the folder doesn't exist yet, create it.
   *
  ...
  private BoxFolder getDropGroupFolder(String dropGroupName) throws BoxSDKServiceException {
    ...
    BoxFolder dropGroupFolder =
        boxSDKService.getFolderWithNameAndParentFolderId(
            folderName, boxSDKService.getBoxSDKServiceConfig().getDropsFolderId());

    if (dropGroupFolder == null) {
      logger.debug("Folder for repository: {} doesn't exist, create it", folderName);
      dropGroupFolder =
          boxSDKService.createSharedFolder(
              folderName, boxSDKService.getBoxSDKServiceConfig().getDropsFolderId());
    }
  ...
  public void exportSourceFile(String bcp47tag, String fileContent) throws DropExporterException {
    ...
    try {
      boxSDKService.uploadFile(
          ...
          getSourceFileName(boxDropExporterConfig.getUploadDate(), bcp47tag),
          fileContent);
  ...
  @Override
  public void exportImportedFile(String filename, String fileContent, String comment)
    ...
    try {
      BoxFile uploadFile =
          boxSDKService.uploadFile(
              boxDropExporterConfig.getImportedFolderId(), filename, fileContent);
  ...
  private void checkIsInitialized() {
    Preconditions.checkNotNull(
        boxDropExporterConfig, getClass().getSimpleName() + " must be initialized first");
  }
}
