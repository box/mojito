 ...
 *
 * <p>If a requested image cannot be found in S3 an attempt will be made to retrieve from the Mojito
 * DB, if the image is found in the DB then an async task will be executed to upload the image to
 * S3.
 *
 * @author maallen
 */
                ...
                () ->
                    databaseImageService
                        ...
                        .map(
                            img -> {
                              logger.debug(
                                  "Found image {} in database, triggering async upload to S3",
                                  img.getName());
                              s3UploadImageAsyncTask.uploadImageToS3(
