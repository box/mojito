...
public interface BranchTextUnitStatisticRepository extends JpaRepository<BranchTextUnitStatistic, Long>, JpaSpecificationExecutor<BranchTextUnitStatistic> {
    ...
    @Query("select new com.box.l10n.mojito.service.branch.BranchTextUnitStatisticWithCounts(btus.id, btus.branchStatistic.id, btus.tmTextUnit.id, btus.forTranslationCount, btus.totalCount) " +
            "from #{#entityName} btus where btus.branchStatistic.id = ?1 and btus.tmTextUnit.id in ?2")
    List<BranchTextUnitStatisticWithCounts> getByBranchStatisticIdAndTmTextUnitIdIn(long branchStatisticId, List<Long> tmTextUnitIds);
}
