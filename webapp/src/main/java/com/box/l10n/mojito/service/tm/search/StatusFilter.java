...
/**
 * To filter text units based on status, status here is not mapping specifically to {@link
 * TMTextUnitVariant.Status} but a combination of this attribute with other text units attributes
 * like {@link TMTextUnitVariant#includedInLocalizedFile}.
 *
 * @author jaurambault
 */
public enum StatusFilter {
  ...
  ALL,
  /** All TextUnits that have a translation with any status ({@link TMTextUnitVariant.Status}. */
  TRANSLATED,
  /** All TextUnits that have no translation */
  UNTRANSLATED,
  /**
   * All TextUnits that have a translation with any status ({@link TMTextUnitVariant.Status} but not
   * rejected.
   */
  TRANSLATED_AND_NOT_REJECTED,
  /**
   * TextUnits with status ({@link TMTextUnitVariant.Status#APPROVED}) or ({@link
   * TMTextUnitVariant.Status#REVIEW_NEEDED}) but not rejected.
   */
   ...
   *
   * <p>Includes text units without translation or with status ({@link
   * TMTextUnitVariant.Status#TRANSLATION_NEEDED}) or not included in file ({@link
