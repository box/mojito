...
public class DropWS {
  ...
  /**
   * Lists all the {@link Drop}s with pagination. It uses View.DropSummary.class to return minimum
   * fields that are annotated with @JsonView(View.DropSummary.class).
   *
   ...
   * @param canceledFilter optionally filter by Drops that have been canceled ( {@code null} no
   *     filter on canceled status, {@code true} get only canceled {@code false} get only not
   *     canceled)
   ...
   *
   * @param exportDropConfig contains information about the drop to be exported
   ...
   *
   * @param importDropConfig contains information about the drop to be re-imported
   ...
   * WS to cancel a drop.
   *
   * @param cancelDropConfig contains information about the drop to be re-imported
   * @return The information about the re-imported drop, updated with a pollable task
   * @throws Exception
   */
  @RequestMapping(
   ...
   * refactor/merged into tm-import function. In any case it should be pollable
   *
  ...
  public ImportXliffBody importXliff(@RequestBody ImportXliffBody importXliffBody)
    ...
    Repository repository =
        repositoryRepository.findById(importXliffBody.getRepositoryId()).orElse(null);
    ...
    if (importXliffBody.isTranslationKit()) {
      updateTMWithLocalizedXLIFF =
          tmService.updateTMWithTranslationKitXLIFF(
              normalizedContent, importXliffBody.getImportStatus(), null);
    } else {
      updateTMWithLocalizedXLIFF =
          tmService.updateTMWithXLIFFByMd5(
              normalizedContent, importXliffBody.getImportStatus(), repository);
    }
