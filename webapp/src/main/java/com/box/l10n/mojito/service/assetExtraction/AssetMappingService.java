 ...
 *
 * @deprecated that is not used anymore but keep it for now for tests which rely on it. Next is to
 *     cleanup those tests.
 * @author jaurambault
 */
@Deprecated
...
public class AssetMappingService {
  ...
  public void mapAssetTextUnitAndCreateTMTextUnit(
    ...
    logger.debug(
        "{} text units were mapped the first time for asset extraction id: {} and tmId: {}",
        mapExactMatches,
        assetExtractionId,
        tmId);

    logger.debug("Create text units for unmapped asset text units");
    List<TMTextUnit> newlyCreatedTMTextUnits =
        createTMTextUnitForUnmappedAssetTextUnitsWithRetry(
    ...
    logger.debug(
        "{} text units were mapped the second time for asset extraction id: {} and tmId: {}",
        mapExactMatches2,
        assetExtractionId,
        tmId);
  ...
  /**
   * Creates {@link TMTextUnit} for {@link AssetTextUnit}s that don't have a MD5 matches.
   *
   * @param assetExtractionId a valid {@link AssetExtraction#id}
   * @param tmId a valid {@link TM#id}
   * @param assetId a valid {@link Asset#id}
   ...
   */
  protected List<TMTextUnit> createTMTextUnitForUnmappedAssetTextUnitsWithRetry(
      final Long assetExtractionId, final Long tmId, final Long assetId, User createdByUser) {
    return retryTemplate.execute(
        new RetryCallback<List<TMTextUnit>, DataIntegrityViolationException>() {
          @Override
          public List<TMTextUnit> doWithRetry(RetryContext context)
              throws DataIntegrityViolationException {

            if (context.getRetryCount() > 0) {
              long mapExactMatches = mapExactMatches(assetExtractionId, tmId, assetId);
              logger.error(
                  "Assume concurrent modification happened, perform remapping: {}",
                  mapExactMatches);
            }

            return createTMTextUnitForUnmappedAssetTextUnits(
  ...
  protected List<TMTextUnit> createTMTextUnitForUnmappedAssetTextUnits(
    ...
    logger.debug(
        "Create TMTextUnit for unmapped AssetTextUnits, assetExtractionId: {} tmId: {}",
        assetExtractionId,
        tmId);
    ...
    for (AssetTextUnit unmappedAssetTextUnit :
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtractionId)) {
  ...
  /**
   * Maps exact matches by mapping {@link AssetTextUnit} to {@link TMTextUnit} based on MD5s for a
   * given {@link AssetExtraction} and {@link TM}.
   *
   * <p>The relationship is 1 to 1 because of the unique constraint on the {@link TMTextUnit}:
   * UK__TM_TEXT_UNIT__MD5__TM_ID. So {@link AssetTextUnit} can be map uniquely to a {@link
   * TMTextUnit}.
   *
   * <p>The select statement looks for all {@link AssetTextUnit}s that are not yet mapped and try to
   * find a match by MD5 in a specified {@link TM}.
   *
  ...
  /**
   * Finds exact matches that are not mapped in {@link AssetTextUnitToTMTextUnit}. It maps {@link
   * AssetTextUnit} to {@link TMTextUnit} based on MD5s for a given {@link AssetExtraction} and
   * {@link TM}.
   *
  ...
  protected int saveExactMatches(List<AssetMappingDTO> exactMatches) {
    ...
    assetTextUnitToTMTextUnits =
        assetTextUnitToTMTextUnitRepository.saveAll(assetTextUnitToTMTextUnits);
