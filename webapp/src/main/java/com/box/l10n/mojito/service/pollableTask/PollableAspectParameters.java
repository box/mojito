 ...
 * Pollable} annotation and the annotated arguments of the wrapped function.
 *
 * @author jaurambault
 */
@Configurable
public class PollableAspectParameters {
  ...
  /**
   * Gets message parameters by looking at the annotation and the function parameters annotated with
   * {@link MsgArg}.
   *
   * @return a map of message parameters, key is the parameter name and the value the argument
   *     value.
   */
  private Map<String, Object> getMessageParams() {
    ...
    List<AnnotatedMethodParam<MsgArg>> annotatedMethodParams =
        aspectJUtils.findAnnotatedMethodParams(pjp, MsgArg.class);

    for (AnnotatedMethodParam<MsgArg> annotatedMethodParam : annotatedMethodParams) {
      params.put(
          annotatedMethodParam.getAnnotation().name(),
          getMsgArgValue(
              annotatedMethodParam.getArg(), annotatedMethodParam.getAnnotation().accessor()));
    }
   ...
   * @return the target itself if no accessor is specified else the value returned by the accessor.
   */
  ...
  /**
   * Sets the the id of the parent task by looking at the annotated parameter with {@link
   * ParentTask}
   */
