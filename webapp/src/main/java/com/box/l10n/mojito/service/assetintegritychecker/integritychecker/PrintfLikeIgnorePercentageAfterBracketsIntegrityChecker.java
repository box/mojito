...
/**
 * '%' symbol preceded by a '}' or ')' are ignored as variable tokens. This can be useful for
 * avoiding false negatives when handling translations that contain percentages inside the
 * translation text.
 *
 * @author mallen
 */
  ...
  private void executeRegexOnPlaceholders(Set<String> placeholders, String regex) {
    ...
    for (String s : placeholders) {
      if (s != null) {
        ...
        while (matcher.find()) {
          throw new RegexCheckerException(
              "Standalone % found, percentages should be doubled (%%) for formatting.");
        }
      }
    }
  }
