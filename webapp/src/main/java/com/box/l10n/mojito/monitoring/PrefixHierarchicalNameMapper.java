...
public class PrefixHierarchicalNameMapper implements HierarchicalNameMapper {
  ...
  public PrefixHierarchicalNameMapper(
      String namePrefix,
      String tagKeyPrefix,
      String stripCharacters,
  ...
  public String toHierarchicalName(Meter.Id id, NamingConvention namingConvention) {
    ...
    String prefix =
        Strings.isNullOrEmpty(namePrefix)
            ? ""
            : (namingConvention.name(namePrefix, Meter.Type.OTHER) + ".");
  ...
  String formatTag(Tag tag) {
    return "."
        + tagKeyPrefix
        + stripCharacters(tag.getKey())
        + "."
        + stripCharacters(tag.getValue());
  }
