...
public class ImportExportTextUnitUtils {
  ...
  /**
   * Gets an {@link ImportExportNote} based on the content of {@link TextUnit} note.
   *
   * <p>The note might not contain JSON or valid JSON. In that case the note content is set in
   * {@link ImportExportNote#sourceComment}. This allows to import files that don't contain JSON,
   * for backward compatibility or just file simplicity.
   *
  ...
  public ImportExportNote getImportExportNote(ITextUnit textUnit) {
    ...
    if (note != null) {
      try {
        ...
        logger.debug(
            "Couldn't convert the note into ImportExportNote, set the whole string as comment");
  ...
  /**
   * Converts the {@link ImportExportNote} to JSON and assigned it to the {@link ITextUnit}.
   *
