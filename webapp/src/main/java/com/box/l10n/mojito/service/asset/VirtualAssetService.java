...
public class VirtualAssetService {
  ...
  public VirtualAsset createOrUpdateVirtualAsset(VirtualAsset virtualAsset)
    ...
    Asset asset =
        assetRepository.findByPathAndRepositoryId(
            virtualAsset.getPath(), virtualAsset.getRepositoryId());
    ...
    if (asset != null && !asset.getVirtual()) {
      ...
      Repository repository =
          repositoryRepository.findById(virtualAsset.getRepositoryId()).orElse(null);

      if (repository == null) {
        throw new VirtualAssetBadRequestException(
            "A valid repository must be provided when creating an asset");
      }
      ...
      AssetExtraction createAssetExtraction =
          assetExtractionService.createAssetExtraction(asset, null);
  ...
  public List<VirtualAssetTextUnit> getTextUnits(long assetId, Boolean doNotTranslateFilter)
    ...
    List<AssetTextUnitDTO> findByAssetExtractionAssetId =
        findByAssetExtractionIdAndDoNotTranslateFilter(
            lastSuccessfulAssetExtractionId, doNotTranslateFilter);

    List<VirtualAssetTextUnit> virtualAssetTextUnits =
        findByAssetExtractionAssetId.stream()
  ...
  public List<VirtualAssetTextUnit> getLocalizedTextUnits(
    ...
    if (repositoryLocale.getParentLocale() == null) {
      ...
      virtualAssetTextUnits =
          getLocalizedTextUnitsForTargetLocale(asset, repositoryLocale, inheritanceMode);
    }

    return virtualAssetTextUnits;
  }

  List<VirtualAssetTextUnit> getLocalizedTextUnitsForTargetLocale(
    ...
    for (AssetTextUnitDTO assetTextUnit : findByAssetExtractionAssetId) {
      ...
      String translation =
          translatorWithInheritance.getTranslation(
              assetTextUnit.getContent(), assetTextUnit.getMd5());

      if (translation == null && InheritanceMode.REMOVE_UNTRANSLATED.equals(inheritanceMode)) {
        logger.debug("Remove untranslated text unit");
      } else {
        logger.debug(
            "Set translation for text unit with name: {}, translation: {}",
            assetTextUnit.getName(),
            translation);

        VirtualAssetTextUnit virtualAssetTextUnit =
            convertAssetTextUnitDTOToVirtualAssetTextUnit(assetTextUnit);
  ...
  public PollableFuture<Void> addTextUnits(
      long assetId, List<VirtualAssetTextUnit> virtualAssetTextUnits)
      throws VirtualAssetRequiredException {
    return updateTextUnits(assetId, virtualAssetTextUnits, false);
  }

  public PollableFuture<Void> replaceTextUnits(
      long assetId, List<VirtualAssetTextUnit> virtualAssetTextUnits)
      throws VirtualAssetRequiredException {
  ...
  public PollableFuture<Void> importLocalizedTextUnits(
      long assetId, long localeId, List<VirtualAssetTextUnit> textUnitForVirtualAssets)
      throws VirtualAssetRequiredException, VirutalAssetMissingTextUnitException {

    logger.debug(
        "Add text unit variants ({}) to virtual assetId: {}",
        textUnitForVirtualAssets.size(),
        assetId);
  ...
  public void deleteTextUnits(Asset asset, List<String> names) {
    ...
    for (String name : names) {
      ...
      List<AssetTextUnit> assetTextUnits =
          assetTextUnitRepository.findByAssetExtractionIdAndName(assetExtractionId, name);

      for (AssetTextUnit assetTextUnit : assetTextUnits) {
        logger.debug(
            "Asset text unit found for name: {} and asset: {}, perform delete",
            name,
            asset.getPath());
  ...
  public TMTextUnitVariant addTextUnitVariant(
      long assetId, long localeId, String name, String content, String comment)
      throws VirtualAssetRequiredException, VirutalAssetMissingTextUnitException {
    ...
    if (textUnitDTOs.isEmpty()) {
      String msg =
          MessageFormat.format("Missing TmTextUnit for assetId: {0} and name: {1}", assetId, name);
      logger.debug(msg);
      throw new VirutalAssetMissingTextUnitException(msg);
    }

    return tmService.addCurrentTMTextUnitVariant(
        textUnitDTOs.get(0).getTmTextUnitId(), localeId, content);
  }

  VirtualAssetTextUnit convertAssetTextUnitDTOToVirtualAssetTextUnit(
      AssetTextUnitDTO assetTextUnitDTO) {
    ...
    if (assetTextUnitDTO.getPluralFormId() != null) {
      virtualAssetTextUnit.setPluralForm(
          pluralFormService.findById(assetTextUnitDTO.getPluralFormId()).getName());
    }
  ...
  void normalizeVirtualAssetTextUnit(VirtualAssetTextUnit virtualAssetTextUnit) {
    ...
    virtualAssetTextUnit.setContent(
        NormalizationUtils.normalize(Strings.nullToEmpty(virtualAssetTextUnit.getContent())));
    virtualAssetTextUnit.setComment(
        NormalizationUtils.normalize(virtualAssetTextUnit.getComment()));
  }
