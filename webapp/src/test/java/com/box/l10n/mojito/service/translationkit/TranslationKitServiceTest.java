...
public class TranslationKitServiceTest extends ServiceTestBase {
  ...
  @Autowired DropService dropService = new DropService();
  ...
  public void testGetXLIFF() {
    ...
    String xliffWithoutIds =
        XliffUtils.replaceXliffVariableContent(translationKitAsXLIFF.getContent());
    ...
    assertEquals(
        "The translation kit must be of type translation",
        TranslationKit.Type.TRANSLATION,
        translationKit.getType());
    assertEquals("There must be 1 TranslationKitTextUnit", 1, findByTranslationKitId.size());

    assertEquals(
        "Check the first TextUnit by name",
        findByTranslationKitId.get(0).getTmTextUnit().getName(),
        "zuora_error_message_verify_state_province");
    assertNull(
        "There shouldn't be any TmTextUnitVariant for this TextUnit",
        findByTranslationKitId.get(0).getExportedTmTextUnitVariant());
  }
  ...
  public void testTranslationNeeded() {
    ...
    assertEquals(
        "The translation kit must be of type review",
        TranslationKit.Type.TRANSLATION,
        translationKit.getType());
    ...
    assertEquals(
        "Check the first TextUnit by name",
        findByTranslationKitId.get(idx).getTmTextUnit().getName(),
        "zuora_error_message_verify_state_province");
    assertEquals(
        "Check the exported TmTextUnitVariant by looking at the content",
        "Veuillez indiquer un état, une région ou une province valide.",
        findByTranslationKitId.get(idx).getExportedTmTextUnitVariant().getContent());

    idx++;
    assertEquals(
        "Check the second TextUnit by name",
        findByTranslationKitId.get(idx).getTmTextUnit().getName(),
        "TEST2");
    assertNull(
        "There shouldn't be any TmTextUnitVariant for this TextUnit",
        findByTranslationKitId.get(idx).getExportedTmTextUnitVariant());
  }
  ...
  public void testCheckEntitiesAndGetTranslationKitExportedAndCurrentTUVs() {
    ...
    logger.debug("Check the translation kit entities");
    List<TranslationKitTextUnit> findByTranslationKitTextUnits =
        translationKitTextUnitRepository.findByTranslationKit(translationKit);
    ...
    assertEquals(
        ...
        translationKit.getType());
    assertEquals(
        "There must be 2 TranslationKitTextUnits", 2, findByTranslationKitTextUnits.size());

    int idx = 0;
    TranslationKitTextUnit translationKitTextUnitZuora = findByTranslationKitTextUnits.get(idx);
    assertEquals(
        "Check that there is a TranslationKitTextUnit for zuora",
        translationKitTextUnitZuora.getTmTextUnit().getName(),
        "zuora_error_message_verify_state_province");
    assertEquals(
        "Check the exported TmTextUnitVariant for zuora",
        "Veuillez indiquer un état, une région ou une province valide.",
        translationKitTextUnitZuora.getExportedTmTextUnitVariant().getContent());

    idx++;
    TranslationKitTextUnit translationKitTextUnitTest2 = findByTranslationKitTextUnits.get(idx);
    assertEquals(
        "Check that there is a TextUnit for TEST2",
        translationKitTextUnitTest2.getTmTextUnit().getName(),
        "TEST2");
    assertNull(
        "There shouldn't be any TmTextUnitVariant for this TextUnit",
        translationKitTextUnitTest2.getExportedTmTextUnitVariant());

    logger.debug(
        "Test GetTranslationKitExportedAndCurrentTUVs and check against previously retreived entities");
    ...
    idx = 0;
    TranslationKitExportedImportedAndCurrentTUV translationKitExportedAndCurrentTUVZuora =
        translationKitExportedAndCurrentTUVs.get(
            translationKitTextUnitZuora.getTmTextUnit().getId());
    assertNotNull("There must be an entry for zuora ", translationKitExportedAndCurrentTUVZuora);
    assertEquals(
        "current TmTextUnitVariant from getTranslationKitExportedAndCurrentTUVs invalid",
        translationKitTextUnitZuora.getExportedTmTextUnitVariant().getId(),
        translationKitExportedAndCurrentTUVZuora.getCurrentTmTextUnitVariant());
    assertEquals(
        "export TmTextUnitVariant from getTranslationKitExportedAndCurrentTUVs invalid",
        translationKitTextUnitZuora.getExportedTmTextUnitVariant().getId(),
        translationKitExportedAndCurrentTUVZuora.getExportedTmTextUnitVariant());

    idx++;
    TranslationKitExportedImportedAndCurrentTUV translationKitExportedAndCurrentTUVTest2 =
        translationKitExportedAndCurrentTUVs.get(
            translationKitTextUnitTest2.getTmTextUnit().getId());
    assertNotNull("There must be an entry for TEST2", translationKitExportedAndCurrentTUVTest2);
    assertNull(
        "current TmTextUnitVariant for TEST2 from getTranslationKitExportedAndCurrentTUVs invalid",
        translationKitExportedAndCurrentTUVTest2.getCurrentTmTextUnitVariant());
    assertNull(
        "export TmTextUnitVariant for TEST2 from getTranslationKitExportedAndCurrentTUVs invalid",
        translationKitExportedAndCurrentTUVTest2.getExportedTmTextUnitVariant());
  }
  ...
  public void testGetTextUnitDTOsForTranslationKitfrFR() {
    ...
    TranslationKit translationKit =
        translationKitService.addTranslationKit(
            dropService.createDrop(tmTestData.repository).getId(),
            tmTestData.frFR.getId(),
            TranslationKit.Type.TRANSLATION);

    List<TextUnitDTO> textUnitDTOsForTranslationKit =
        translationKitService.getTextUnitDTOsForTranslationKit(
            translationKit.getId(), TranslationKit.Type.TRANSLATION);
  ...
  public void testGetTextUnitDTOsWithInheritanceForTranslationKitfrCA() {
    ...
    List<TextUnitDTO> textUnitDTOsForTranslationKit =
        translationKitService.getTextUnitDTOsForTranslationKitWithInheritance(
