...
public class PseudoLocalizationTest {
  ...
  public void testStringIsConvertedToDiacritics() {
    ...
    assertNotEquals(
        "The string should be converted to diacritics", "English Sentence", diacriticsString);
  }
  ...
  public void testConvertMessageFormatStringToPseudoLoc() {
    ...
    String pseudoLocalized =
        ps.convertStringToPseudoLoc(
            "English Sentence with {placeholder1} and {placeholder2} which should not be pseudolocalized",
            checkers);
    assertTrue(
        "The placeholder should not be pseudolocalized",
        pseudoLocalized.contains("{placeholder1}"));
    assertTrue(
        "The placeholder should not be pseudolocalized",
        pseudoLocalized.contains("{placeholder2}"));
  }
  ...
  public void testConvertPluralMessageFormatStringToPseudoLoc1() {
    ...
    Set<TextUnitIntegrityChecker> checkers = new HashSet<>();
    checkers.add(new MessageFormatIntegrityChecker());
    // v and f map to single unicode character in pseudolocalization which makes it easier to test
    // and verify
    String pseudoLocalized =
        ps.convertStringToPseudoLoc(
            "Test {count, plural, one {# v} other {# vf}} with Plurals", checkers);
    assertTrue(
        "The plural syntax should not be pseudolocalized",
        pseudoLocalized.contains("count, plural, one"));
    assertTrue(
        "The plural syntax should not be pseudolocalized", pseudoLocalized.contains("other"));
    assertTrue(
        "The plural text variation should be pseudolocalized", pseudoLocalized.contains("{# ν}"));
    assertTrue(
        "The plural text variation should be pseudolocalized", pseudoLocalized.contains("{# νƒ}"));
  }
  ...
  public void testConvertPluralMessageFormatStringToPseudoLoc2() {
    ...
    String pseudoLocalized =
        ps.convertStringToPseudoLoc(
            "Viewed by {numUsers, plural, one {1 v} other {{numUsers} vf}} and others", checkers);
    assertFalse("The string should be pseudolocalized", pseudoLocalized.contains("Viewed by"));
    assertTrue(
        "The plural syntax should not be pseudolocalized",
        pseudoLocalized.contains("numUsers, plural, one"));
    assertTrue(
        "The plural syntax should not be pseudolocalized", pseudoLocalized.contains("other"));
    assertTrue(
        "The plural text variation should be pseudolocalized", pseudoLocalized.contains("{1 ν}"));
    assertTrue(
        "The plural text variation should be pseudolocalized while the placeholder should not",
        pseudoLocalized.contains("{{numUsers} νƒ}"));
  }
  ...
  public void testConvertPluralMessageFormatStringToPseudoLoc3() {
    ...
    String pseudoLocalized =
        ps.convertStringToPseudoLoc(
            "{count, plural, one {# Comment or Task} other {# Comments or Tasks}}", checkers);
    assertTrue(
        "The plural syntax should not be pseudolocalized",
        pseudoLocalized.contains("count, plural, one"));
    assertTrue(
        "The plural syntax should not be pseudolocalized", pseudoLocalized.contains("other"));
    assertFalse(
        "The plural text variation should be pseudolocalized",
        pseudoLocalized.contains("{# Comment or Task}"));
    assertFalse(
        "The plural text variation should be pseudolocalized while the placeholder should not",
