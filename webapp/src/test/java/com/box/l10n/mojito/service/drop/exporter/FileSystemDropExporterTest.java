...
public class FileSystemDropExporterTest extends ServiceTestBase {
  ...
  public void all() throws DropExporterException, DropImporterException, BoxSDKServiceException {
    ...
    FileSystemDropImporter fileSystemDropImporter =
        (FileSystemDropImporter) recreate.getDropImporter();
    assertEquals(
        recreate.getFileSystemDropExporterConfig().getDropFolderPath() + "/Localized Files",
        fileSystemDropImporter.getLocalizedFolderPath().toString());
    ...
    File sourceFilesDirectory =
        Paths.get(
                recreate.getFileSystemDropExporterConfig().getDropFolderPath(),
    ...
    assertEquals(
        "Must be 1 file for the exported content and 1 for comments",
        1,
        filesInImportedFilesDirectory.length);
