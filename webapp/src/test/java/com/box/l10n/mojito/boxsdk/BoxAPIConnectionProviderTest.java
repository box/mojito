...
public class BoxAPIConnectionProviderTest {
  ...
  @InjectMocks BoxAPIConnectionProvider boxAPIConnectionProvider;
  ...
  public void testGetConnectionWillReuseConnection() throws BoxSDKServiceException {
    ...
    logger.debug(
        "Box API connection should only be created once even though getConnection was called multiple times");
    Mockito.verify(providerSpy, Mockito.times(1)).createBoxAPIConnection();
  }

  @Test
  public void testGetConnectionWillCreateNewConnectionIfConfigChanges()
    ...
    logger.debug(
        "Get connection 3nd time will have a different config, so should create a new connection");
