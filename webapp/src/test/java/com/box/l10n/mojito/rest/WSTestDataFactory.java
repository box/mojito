...
public class WSTestDataFactory {
  ...
  @Autowired XliffDataFactory xliffDataFactory;
   ...
   */
  @Transactional
  public Repository createRepository(TestIdWatcher testIdWatcher)
  ...
  /**
   * Creates a repo supporting few languages, a TM, processes an asset to add text units and add
   * some variants.
   *
   * @param testIdWatcher
   * @return The {@link Repository}
   */
  public Repository createRepoAndAssetAndTextUnitsAndVariants(TestIdWatcher testIdWatcher)
      throws Exception {
    ...
    for (TMTextUnit tmTextUnit : tmTextUnits) {
      ...
      tmService.addCurrentTMTextUnitVariant(
          tmTextUnitId,
          frFrLocale.getId(),
          "Variant for tmTextUnit " + tmTextUnitId + " - " + frFrLocale.getBcp47Tag());
    }
