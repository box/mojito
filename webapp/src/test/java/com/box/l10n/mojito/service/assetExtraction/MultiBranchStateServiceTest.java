...
public class MultiBranchStateServiceTest extends ServiceTestBase {
    ...
    public void getMultiBranchStateForAssetExtractionId() throws InterruptedException {
        ...
        Optional<MultiBranchState> multiBranchStateForAssetExtractionBefore = multiBranchStateBlobStorage.getMultiBranchStateForAssetExtractionId(tmTestData.assetExtraction.getId(), 0L);
        assertFalse(multiBranchStateForAssetExtractionBefore.isPresent());

        MultiBranchState multiBranchStateForAssetExtractionId = multiBranchStateService.getMultiBranchStateForAssetExtractionId(tmTestData.assetExtraction.getId(), 0L);

        MultiBranchState expectedMultiBranchState = MultiBranchState.of()
                .withBranches(ImmutableSet.of(Branch.builder()
                ...
                .withBranchStateTextUnits(
                        ImmutableList.of(
                                convertTmTextUnitToBranchStateTextUnit(tmTestData.createAssetTextUnit1, tmTestData.addTMTextUnit1.getId()),
                                convertTmTextUnitToBranchStateTextUnit(tmTestData.createAssetTextUnit2, tmTestData.addTMTextUnit2.getId())
                        ));
        expectedMultiBranchState = roundDateTimesIfMysql(expectedMultiBranchState);

        Assertions.assertThat(multiBranchStateForAssetExtractionId).usingRecursiveComparison().isEqualTo(expectedMultiBranchState);

        Optional<MultiBranchState> multiBranchStateForAssetExtractionAfter = multiBranchStateBlobStorage.getMultiBranchStateForAssetExtractionId(tmTestData.assetExtraction.getId(), 0L);
     ...
     * <p>
     * On current Mysql schema as precision of second for dates, the test fails with date precision issue without rounding here
     * - ignore for now
     * - update the schema to have proper precision later
     *
    ...
    MultiBranchState roundDateTimesIfMysql(MultiBranchState expectedMultiBranchState) {
        ...
        if (dbUtils.isMysql()) {
            expectedMultiBranchState = expectedMultiBranchState.withBranchStateTextUnits(expectedMultiBranchState.getBranchStateTextUnits().stream()
                    .map(bstu -> bstu.withCreatedDate(roundDateTimeToSecond(bstu.getCreatedDate())))
                    .collect(ImmutableList.toImmutableList()));

            expectedMultiBranchState = expectedMultiBranchState.withBranches(expectedMultiBranchState.getBranches().stream()
    ...
    BranchStateTextUnit convertTmTextUnitToBranchStateTextUnit(AssetTextUnit assetTextUnit, Long tmTextUnitId) {
