...
/**
 * Not a test but a set of tools to generate locale and plural forms from CLDR that are not yet in
 * Mojito
 *
...
public class PluralFormForLocaleServiceTest extends ServiceTestBase {
  ...
  @Autowired PluralFormForLocaleService pluralFormForLocaleService;
  ...
  public void generateNewLocaleFromCLDR() throws IOException {
    JsonNode languagesEn001 =
        objectMapper.readTree(new File("node_modules/cldr-data/main/en-001/languages.json"));
    JsonNode availableLocales =
        objectMapper.readTree(new File("node_modules/cldr-data/availableLocales.json"));

    List<Locale> currentMojitoLocales = localeRepository.findAll();
    List<String> currentMojitoBcp47 =
        currentMojitoLocales.stream().map(l -> l.getBcp47Tag()).collect(Collectors.toList());
  ...
  public void generateNewPluralFormsForExistingLocales() {
    ...
    List<PluralFormForLocale> pluralFormsForLocales =
        pluralFormForLocaleService.getPluralFormsForLocales();

    for (PluralFormForLocale pluralFormsForLocale : pluralFormsForLocales) {
      PluralFormForLocale findByLocale =
          pluralFormForLocaleRepository.findByLocaleAndPluralForm(
              pluralFormsForLocale.getLocale(), pluralFormsForLocale.getPluralForm());

      if (findByLocale == null) {
        PluralFormForLocale savedPluralFormForLocale =
            pluralFormForLocaleRepository.save(pluralFormsForLocale);
        ...
        logger.info(
            "insert into plural_form_for_locale (locale_id, plural_form_id) values ({}, {});",
