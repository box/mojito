...
public class TranslationKitStepTest {
  ...
  public void testHandleStartDocument() {
    ...
    assertNotNull(
        "TranslationKitTextUnits must have been initialized",
        translationKitStep.translationKitTextUnits);
  }
  ...
  public void testHandleTextUnit() {
    ...
    assertEquals(
        "translationKitId must match what was in the TextUnitDTO",
        translationKitId,
        translationKitTextUnit.getTranslationKit().getId());
    assertEquals(
        "tmTextUnitId must match what was in the TextUnitDTO",
        tmTextUnitId,
        translationKitTextUnit.getTmTextUnit().getId());
    assertEquals(
        "tmTextUnitVariantId of entites must match what was in the TextUnitDTO",
        tmTextUnitVariantId,
        translationKitTextUnit.getExportedTmTextUnitVariant().getId());
  }
  ...
  public void testHandleEndDocument() {
    ...
    assertEquals("the input event should be returned", event, result);
    verify(mockTranslationKitService)
        .updateTranslationKitWithTmTextUnits(translationKitId, translationKitTextUnits, 0L);
  }
  ...
  public void testGetTMTextUnitVariant() {
    ...
    assertEquals(
        "The tmTextUnitVariant#Id extracted from the text unit must be the same as the one set in the TextUnitDTO",
        tmTextUnitVariantId,
        result.getId());
  }
}
