...
public class PlaceholderPatternTypeTest {
  ...
  @ParameterizedTest
  @ValueSource(strings = {"%s", "%s ", " %s ", " test %s test", " test \n\t %s "})
  ...
  @ParameterizedTest
  @ValueSource(strings = {"%,d", "%,d ", " %,d ", " test %,d test", " test \n\t %,d \n\t test "})
  ...
  public void testMessageFormatDoubleEmptyPattern(String text) {
    Pattern pattern =
        Pattern.compile(PlaceholderPatternType.MESSAGE_FORMAT_DOUBLE_EMPTY.getValue());
    Matcher matcher = pattern.matcher(text);
    Assertions.assertTrue(matcher.find());
    Assertions.assertEquals("{{}}", matcher.group());
  }
