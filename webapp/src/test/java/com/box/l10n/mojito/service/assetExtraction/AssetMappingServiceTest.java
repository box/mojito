...
public class AssetMappingServiceTest extends ServiceTestBase {
  ...
  @Test
  public void testUpdateExactMatchesAndGetUnmappedAssetTextUnits()
    ...
    Repository testRepository =
        repositoryService.createRepository(
            testIdWatcher.getEntityName("testUpdateExactMatchesAndGetUnmappedAssetTextUnits"));
    TM tm = testRepository.getTm();
    Asset testAsset =
        assetService.createAssetWithContent(
            testRepository.getId(), "fake_for_test_real_asset", "fake for test real tm repo");
    TMTextUnit addTMTextUnit1 =
        tmService.addTMTextUnit(tm.getId(), testAsset.getId(), "TEST1", "Content1", "Comment1");
    TMTextUnit addTMTextUnit2 =
        tmService.addTMTextUnit(tm.getId(), testAsset.getId(), "TEST2", "Content2", "Comment2");
    ...
    AssetTextUnit createAssetTextUnit1 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction, "TEST1", "Content1", "Comment1");
    AssetTextUnit createAssetTextUnit2 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction, "TEST2", "Content2", "Comment2");
    AssetTextUnit createAssetTextUnit3 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction, "TEST3", "Content3", "Comment3");

    logger.debug("Done creating data for test, start testing");

    assertEquals(
        "Make sure no AssetTextUnit are mapped",
        3,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction.getId()).size());

    assetMappingService.mapExactMatches(assetExtraction.getId(), tm.getId(), testAsset.getId());

    assertEquals(
        "We should now have 1 AssetTextUnit mapped",
        1,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction.getId()).size());
    ...
    assertEquals(
        "Check for mapping reccords", addTMTextUnit1.getId(), next.getTmTextUnit().getId());
    assertEquals(
        "Check for mapping reccords",
        createAssetTextUnit1.getId(),
        next.getAssetTextUnit().getId());

    next = iterator.next();
    assertEquals(
        "Check for mapping reccords", addTMTextUnit2.getId(), next.getTmTextUnit().getId());
    assertEquals(
        "Check for mapping reccords",
        createAssetTextUnit2.getId(),
        next.getAssetTextUnit().getId());

    assertFalse("There shouldn't be any other mapping reccords", iterator.hasNext());

    logger.debug(
        "Running twice updateExactMatches should not change anything because everything have been mapped already");
    assetMappingService.mapExactMatches(assetExtraction.getId(), tm.getId(), testAsset.getId());
    ...
    TMTextUnit addTMTextUnit3 =
        tmService.addTMTextUnit(tm.getId(), testAsset.getId(), "TEST3", "Content3", "Comment3");

    logger.debug("Re-map, a new entry should be added");
    assetMappingService.mapExactMatches(assetExtraction.getId(), tm.getId(), testAsset.getId());

    assertEquals(
        "All AssetTextUnit must be mapped now",
        0,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction.getId()).size());
  ...
  @Test
  public void testCreateTMTextUnitForUnmappedAssetTextUnits()
      throws RepositoryNameAlreadyUsedException {
    ...
    Repository testRepository =
        repositoryService.createRepository(
            testIdWatcher.getEntityName("testCreateTMTextUnitForUnmappedAssetTextUnits"));
    TM tm = testRepository.getTm();
    Asset asset =
        assetService.createAssetWithContent(
            testRepository.getId(), "test-asset-path", "test asset content");
    ...
    assertEquals(
        "Nothing should be mapped yet",
        3,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction.getId()).size());
    assertEquals(
        "No TMTextUnit should be present in the TM",
        0,
        tmTextUnitRepository.findByTm_id(tm.getId()).size());

    assetMappingService.createTMTextUnitForUnmappedAssetTextUnitsWithRetry(
        assetExtraction.getId(), tm.getId(), asset.getId(), null);

    assertEquals(
        "A TMTextUnit should have been created for each AssetTextUnit",
        3,
        tmTextUnitRepository.findByTm_id(tm.getId()).size());
  }

  @Test
  public void testCreateTMTextUnitForUnmappedAssetTextUnitsIncremental()
      throws RepositoryNameAlreadyUsedException {
    ...
    assertEquals(
        "A TMTextUnit should have been created",
        1,
        tmTextUnitRepository.findByTm_id(tm.getId()).size());
    ...
    assertEquals(
        "There should be 2 new unmapped AssetTextUnit",
        2,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction.getId()).size());
    assertEquals(
        "There should be still a single TMTextUnit",
        1,
        tmTextUnitRepository.findByTm_id(tm.getId()).size());
  ...
  public void testMap()
    ...
    // This is the actual data that should be proccessed
    Repository testRepository =
        repositoryService.createRepository(testIdWatcher.getEntityName("testMap"));
    ...
    Asset asset =
        assetService.createAssetWithContent(
            testRepository.getId(), "fake_for_test", "fake for test");
    ...
    assertEquals(
        "Last successfulAssetExtraction should be set", asset.getId(), updatedAsset.getId());

    logger.debug(
        "Perform mapping on a second asset extraction that has more AssetTextUnit with same TM (what will happen when we keep updating documents)");
    ...
    assetExtractionService.createAssetTextUnit(
        assetExtraction2, "TEST2B", "Content2B", "Comment2B");
    assetExtractionService.createAssetTextUnit(
    ...
    assertEquals(
        "All AssetTextUnit must be mapped",
        0,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction2.getId()).size());
    ...
    logger.debug(
        "Perform mapping on a third asset extraction where some AssetTextUnit were removed with same TM (what will happen when we keep updating documents and removing some resources)");
    ...
    assetExtractionService.createAssetTextUnit(
        assetExtraction3, "TEST3A", "Content3A", "Comment3A");
    assetExtractionService.createAssetTextUnit(
    ...
    assertEquals(
        "All AssetTextUnit must be mapped",
        0,
        assetTextUnitRepository.getUnmappedAssetTextUnits(assetExtraction3.getId()).size());
  ...
  @Test
  public void testSourceLeveraging()
      throws RepositoryLocaleCreationException, RepositoryNameAlreadyUsedException {
    ...
    // This is the actual data that should be proccessed
    Repository testRepository =
        repositoryService.createRepository(testIdWatcher.getEntityName("testSourceLeveraging"));
    repositoryService.addRepositoryLocale(testRepository, "fr-FR");
    repositoryService.addRepositoryLocale(testRepository, "ja-JP");
    ...
    assetExtraction.setAsset(asset);
    assetExtraction = assetExtractionRepository.save(assetExtraction);
    AssetTextUnit assetTextUnitForLeveraging =
        assetExtractionService.createAssetTextUnit(
            assetExtraction, "TEST1A", "Content1A", "Comment1A");
    ...
    logger.debug(
        "Add initial translation initial to test leveraging (this translation will be copied from 1 TMTextUnit to another");
    TMTextUnit tmTextUnitForLeveraging =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging.getMd5());
    TMTextUnitVariant translationfrFR =
        tmService.addCurrentTMTextUnitVariant(
            tmTextUnitForLeveraging.getId(),
            localeService.findByBcp47Tag("fr-FR").getId(),
            "Content1Afr-FR");
    TMTextUnitVariant translationjaJP =
        tmService.addCurrentTMTextUnitVariant(
            tmTextUnitForLeveraging.getId(),
            localeService.findByBcp47Tag("ja-JP").getId(),
            "Content1Aja-JP");

    logger.debug("Done creating data for test, start testing");

    logger.debug(
        "Perform mapping where some AssetTextUnit were change to test name+content (comment change only) leveraging");

    AssetExtraction assetExtraction2 = new AssetExtraction();
    assetExtraction2.setAsset(asset);
    assetExtraction2 = assetExtractionRepository.save(assetExtraction2);
    AssetTextUnit assetTextUnitForLeveraging2 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction2, "TEST1A", "Content1A", "Comment1B");
    assetExtractionService.createAssetTextUnit(
        assetExtraction2, "TEST2A", "Content2A", "Comment2A");
    ...
    TMTextUnit tmTextUnitForLeveraging2 =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging2.getMd5());
    ...
    TMTextUnitVariantComment comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation2frFR.getTmTextUnitVariantId())
            .get(0);
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation2jaJP.getTmTextUnitVariantId())
            .get(0);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by name and content for source leveraging - leveraging from tmTextUnitId: "
    ...
    assetExtraction3.setAsset(asset);
    assetExtraction3 = assetExtractionRepository.save(assetExtraction3);
    AssetTextUnit assetTextUnitForLeveraging3 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction3, "TEST1A", "Content1B", "Comment1B");
    assetExtractionService.createAssetTextUnit(
    ...
    TMTextUnit tmTextUnitForLeveraging3 =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging3.getMd5());
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation3frFR.getTmTextUnitVariantId())
            .get(1);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        ...
        comment.getContent());
    assertEquals(
        "The source text has changed, hence it needs review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation3frFR.getStatus());
    ...
    assertEquals(
        "The source text has changed, hence it needs review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation3jaJP.getStatus());

    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation3jaJP.getTmTextUnitVariantId())
            .get(1);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by name for source leveraging - leveraging from tmTextUnitId: "
    ...
    AssetTextUnit assetTextUnitForLeveraging4 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction4, "TEST3A", "Content1B", "Comment3A");
    assetExtractionService.createAssetTextUnit(
    ...
    TMTextUnit tmTextUnitForLeveraging4 =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging4.getMd5());
    ...
    assertEquals(
        "The text unit used for leveraging needed review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation4frFR.getStatus());

    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation4frFR.getTmTextUnitVariantId())
            .get(2);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
    ...
    assertEquals(
        "The text unit used for leveraging needed review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation4jaJP.getStatus());

    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation4jaJP.getTmTextUnitVariantId())
            .get(2);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    logger.debug(
        "Perform mapping where some AssetTextUnit were change to test different kind of leveraging");
    ...
    AssetTextUnit assetTextUnitForLeveraging5 =
        assetExtractionService.createAssetTextUnit(
            assetExtraction5, "TEST3A", "Content3A", "Comment3A");
    AssetTextUnit assetTextUnitForLeveraging5b =
        assetExtractionService.createAssetTextUnit(
            assetExtraction5, "TEST4A", "Content1B", "Comment4A");
    AssetTextUnit assetTextUnitForLeveraging5c =
        assetExtractionService.createAssetTextUnit(
    ...
    TMTextUnit tmTextUnitForLeveraging5 =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging5.getMd5());
    TMTextUnit tmTextUnitForLeveraging5b =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging5b.getMd5());
    TMTextUnit tmTextUnitForLeveraging5c =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging5c.getMd5());
    ...
    assertEquals(
        "The source text has changed, hence it needs review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation5frFR.getStatus());

    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5frFR.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by name for source leveraging - leveraging from tmTextUnitId: "
    ...
    assertEquals(
        "The source text has changed, hence it needs review",
        TMTextUnitVariant.Status.TRANSLATION_NEEDED,
        translation5jaJP.getStatus());

    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5jaJP.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by name for source leveraging - leveraging from tmTextUnitId: "
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5frFRb.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5jaJPb.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5frFRc.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation5jaJPc.getTmTextUnitVariantId())
            .get(3);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    AssetTextUnit assetTextUnitForLeveraging6 =
        assetExtractionService.createAssetTextUnit(
    ...
    TMTextUnit tmTextUnitForLeveraging6 =
        tmTextUnitRepository.findFirstByTmAndMd5(tm, assetTextUnitForLeveraging6.getMd5());
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation6frFR.getTmTextUnitVariantId())
            .get(4);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
    ...
    comment =
        tmTextUnitVariantCommentRepository
            .findAllByTmTextUnitVariant_id(translation6jaJP.getTmTextUnitVariantId())
            .get(4);
    assertEquals(TMTextUnitVariantComment.Type.LEVERAGING, comment.getType());
    assertEquals(
        "by content for source leveraging - leveraging from tmTextUnitId: "
  ...
  private void checkMapping(
      AssetTextUnitToTMTextUnit next, AssetExtraction assetExtraction, TM tm) {
    assertEquals(
        "Check for mapping reccords",
        next.getAssetTextUnit().getName(),
        next.getTmTextUnit().getName());
    assertEquals(
        "Check for mapping reccords",
        next.getAssetTextUnit().getContent(),
        next.getTmTextUnit().getContent());
    assertEquals(
        "Check for mapping reccords",
        next.getAssetTextUnit().getComment(),
        next.getTmTextUnit().getComment());
    assertEquals(
        "Check for mapping reccords",
        assetExtraction.getId(),
        next.getAssetTextUnit().getAssetExtraction().getId());
  ...
  protected void createOtherTMAssetAndTextUnit(
      String repoName,
      String assetContent,
      String assetPath,
      String tuName,
    ...
    Repository otherRepo =
        repositoryService.createRepository(testIdWatcher.getEntityName(repoName));
    TM otherTM = otherRepo.getTm();
    Asset otherAsset =
        assetService.createAssetWithContent(otherRepo.getId(), assetPath, assetContent);
