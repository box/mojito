...
public class PushRunServiceTest extends ServiceTestBase {
    ...
    @Autowired
    PushRunAssetService pushRunAssetService;

    @Autowired
    PushRunAssetTmTextUnitRepository pushRunAssetTmTextUnitRepository;

    @Autowired
    PushRunAssetTmTextUnitService pushRunAssetTmTextUnitService;
    ...
    public void testClearPushRunLinkedData() {
        ...
        TMTextUnit tmTextUnit = tmService.addTMTextUnit(tm.getId(),
                                                        asset.getId(),
                                                        "hello_world",
                                                        "Hello World!",
                                                        "Comments about hello world");
        pushRunAssetTmTextUnitService.createPushRunAssetTmTextUnit(pushRunAsset, tmTextUnit);
        Assert.assertFalse(pushRunAssetTmTextUnitRepository.findByPushRunAsset(pushRunAsset,
        ...
        Assert.assertTrue(pushRunAssetTmTextUnitRepository.findByPushRunAsset(pushRunAsset,
                                                                              PageRequest.of(0, Integer.MAX_VALUE))
                                  .isEmpty());
    }
    ...
    public void testAssociatePushRunToTextUnitsWithExistingData() {
        PushRun pushRun = pushRunService.createPushRun(repository, "testAssociatePushRunToTextUnitsWithExistingData");
        Assert.assertTrue(pushRunAssetRepository.findByPushRun(pushRun).isEmpty());
    ...
    public void testAssociatePushRunToTextUnitIds() {
        ...
        pushRunService.associatePushRunToTextUnitIds(pushRun, asset, Arrays.asList(tmTextUnit1.getId(),
                                                                                   tmTextUnit2.getId()));
        List<TMTextUnit> textUnits = pushRunService.getPushRunTextUnits(pushRun, PageRequest.of(0, Integer.MAX_VALUE));
        Assert.assertEquals(2, textUnits.size());
    }
