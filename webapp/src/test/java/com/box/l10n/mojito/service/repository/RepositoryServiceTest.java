...
public class RepositoryServiceTest extends ServiceTestBase {
  ...
  public void testAddRootLocaleUnique() throws Exception {
    ...
    try {
      ...
      Assert.fail(
          "It shouldn't be possible to add twice a root locale (root locale was added during repository creation)");
    } catch (RuntimeException re) {
      assertEquals(
          "Root locale already exists in repository [" + repository.getId() + "]", re.getMessage());
    }
  }
  ...
  public void testUpdateRepositoryLocalesForEmptySet() throws Exception {
    ...
    assertEquals(
        1, repositoryRepository.findByName(repository.getName()).getRepositoryLocales().size());
    assertEquals(
        1, repositoryRepository.findByName(repository2.getName()).getRepositoryLocales().size());
  }
  ...
  public void testUpdateRepositoryLocales() throws Exception {
    ...
    repositoryLocales.add(
        new RepositoryLocale(repository, localeService.findByBcp47Tag("fr-FR"), true, null));
    ...
    assertEquals(
        "This number includes the root locale",
        2,
        repositoryRepository.findByName(repository.getName()).getRepositoryLocales().size());
    assertEquals(
        "Repository is not expected to be changed",
        1,
        repositoryRepository
            .findByName(controlledRepository.getName())
            .getRepositoryLocales()
            .size());

    repositoryLocales.clear();
    RepositoryLocale zhTW =
        new RepositoryLocale(repository, localeService.findByBcp47Tag("zh-TW"), true, null);
    repositoryLocales.add(zhTW);
    repositoryLocales.add(
        new RepositoryLocale(repository, localeService.findByBcp47Tag("zh-HK"), true, zhTW));
    ...
    assertEquals(
        "This number includes the root locale", 3, repository1.getRepositoryLocales().size());
    for (RepositoryLocale repositoryLocale : repository1.getRepositoryLocales()) {
      assertNotEquals(
          "fr-FR should have been removed", "fr-FR", repositoryLocale.getLocale().getBcp47Tag());
    }
  ...
  public void testDeleteRepository() throws Exception {
    ...
    assertEquals(
        "The repository should be found because it is not deleted yet",
        1,
        repositoryRepository
            .findAll(where(ifParamNotNull(nameEquals(repositoryName))).and(deletedEquals(false)))
            .size());
    ...
    assertEquals(
        "The deleted repository is found",
        0,
        repositoryRepository
            .findAll(where(ifParamNotNull(nameEquals(repositoryName))).and(deletedEquals(false)))
            .size());
    assertEquals(
        numberOfRepositories - 1, repositoryRepository.findByDeletedFalseOrderByNameAsc().size());
    ...
    assertTrue(
        "The deleted repository is not renamed properly",
        deletedRepository.getName().startsWith("deleted__"));
    ...
    assertEquals(
        "The repository should be found when it is added again",
        1,
        repositoryRepository
            .findAll(where(ifParamNotNull(nameEquals(repositoryName))).and(deletedEquals(false)))
            .size());
    assertEquals(
        numberOfRepositories, repositoryRepository.findByDeletedFalseOrderByNameAsc().size());
    assertEquals(
        "The repository is created with unexpected name",
        repositoryName,
        repositoryToCreateAgain.getName());
    assertNotEquals(
        "The repository is created with existing id",
        repositoryId,
        repositoryToCreateAgain.getId());
