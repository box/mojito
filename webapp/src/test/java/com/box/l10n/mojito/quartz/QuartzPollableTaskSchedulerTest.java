...
public class QuartzPollableTaskSchedulerTest  extends ServiceTestBase {
    ...
    public void test() throws ExecutionException, InterruptedException {
        PollableFuture<AQuartzPollableJobOutput> pollableFuture = quartzPollableTaskScheduler.scheduleJob(AQuartzPollableJob.class, 10L);
        AQuartzPollableJobOutput s = pollableFuture.get();
        assertEquals("output: 10", s.getOutput());

        PollableFuture<AQuartzPollableJobOutput> stringPollableFuture2 = quartzPollableTaskScheduler.scheduleJob(AQuartzPollableJob.class, 10L);
    ...
    public void testVoid() throws ExecutionException, InterruptedException {
        PollableFuture<Void> pollableFuture = quartzPollableTaskScheduler.scheduleJob(VoidQuartzPollableJob.class, 10L);
        Void aVoid = pollableFuture.get();
        assertEquals(null, aVoid);
        try {
            Object output = pollableTaskBlobStorage.getOutputJson(pollableFuture.getPollableTask().getId());
            fail();
    ...
    public static class AQuartzPollableJob2 extends QuartzPollableJob<Long, AQuartzPollableJobOutput> {
        @Override
