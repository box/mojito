...
public class RepositoryWSTest extends WSTestBase {
    ...
    @Autowired
    WSTestDataFactory wsTestDataFactory;
    ...
    @Test
    public void testGetRepositoryByName() throws RestClientException, RepositoryNameAlreadyUsedException {
        Repository expectedRepository = wsTestDataFactory.createRepository(testIdWatcher);
        com.box.l10n.mojito.rest.entity.Repository actualRepository = repositoryClient.getRepositoryByName(expectedRepository.getName());

        assertRepositoriesAreEqual(expectedRepository, actualRepository);
    }

    @Test
    public void testGetRepositoryById() throws RepositoryNotFoundException, RepositoryNameAlreadyUsedException {
        Repository expectedRepository = wsTestDataFactory.createRepository(testIdWatcher);
    ...
    public void testUpdateRepositoryLocales() throws RepositoryNotFoundException, RepositoryNameAlreadyUsedException, ResourceNotUpdatedException {
        ...
        assertEquals("Description should have remained the same", expectedRepository.getDescription(), actualRepository.getDescription());
        assertEquals("Should only have de-DE and one root locale", 2, actualRepository.getRepositoryLocales().size());
        ...
        assertEquals("Name should have remained the same", expectedRepository.getName(), actualRepository.getName());
        assertEquals("Description should have remained the same", expectedRepository.getDescription(), actualRepository.getDescription());
        assertEquals("Should only have five locales including the roolt locale", 5, actualRepository.getRepositoryLocales().size());
        for (RepositoryLocale repositoryLocale : actualRepository.getRepositoryLocales()) {
    ...
    protected void assertRepositoriesAreEqual(Repository expectedRepository, com.box.l10n.mojito.rest.entity.Repository actualRepository) {
        ...
        for (com.box.l10n.mojito.entity.RepositoryLocale repositoryLocale : expectedRepository.getRepositoryLocales()) {
