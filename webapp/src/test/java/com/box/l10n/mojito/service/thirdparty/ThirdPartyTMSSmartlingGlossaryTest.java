...
public class ThirdPartyTMSSmartlingGlossaryTest {
  ...
  @Mock SmartlingClient mockSmartlingClient;

  @Mock AssetRepository mockAssetRepository;

  @Mock AssetTextUnitRepository mockAssetTextUnitRepository;

  @Mock Repository mockRepository;

  @Mock Asset mockAsset;

  @Mock AssetExtraction mockAssetExtraction;

  @Captor ArgumentCaptor<List<VirtualAssetTextUnit>> textUnitsCaptor;

  @Captor ArgumentCaptor<Long> localeIdCaptor;

  @Captor ArgumentCaptor<Long> assetIdCaptor;
  ...
  public void setup() throws IOException, VirtualAssetBadRequestException {
    ...
    Files.readLines(
            new File("src/test/resources/com/box/l10n/mojito/service/thirdparty/Test_Glossary.tbx"),
    ...
    doReturn(glossaryDetails)
        .when(mockSmartlingClient)
        .getGlossaryDetails(anyString(), anyString());
    doReturn(testTBXFileString)
        .when(mockSmartlingClient)
        .downloadGlossaryFile(anyString(), anyString());
    doReturn(testTBXFileString)
        .when(mockSmartlingClient)
        .downloadSourceGlossaryFile(anyString(), anyString(), anyString());
    doReturn(testTBXFileString)
        .when(mockSmartlingClient)
        .downloadGlossaryFileWithTranslations(anyString(), anyString(), anyString(), anyString());
    doReturn(virtualAsset)
        .when(mockVirtualAssetService)
        .createOrUpdateVirtualAsset(isA(VirtualAsset.class));
    ...
    doReturn(assetTextUnitList)
        .when(mockAssetTextUnitRepository)
        .findByAssetExtractionId(anyLong());
  }

  @Test
  public void testPullSourceDeletesExistingAndImportsTextUnits()
    ...
    verify(mockSmartlingClient, times(1))
        .downloadSourceGlossaryFile("testAccountId", "someUid", "en-US");
    ...
    assertTrue(
        virtualAssetTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("school")
    ...
    assertTrue(
        virtualAssetTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("house")
    ...
    assertTrue(
        virtualAssetTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("field")
  ...
  public void testExceptionThrownIfErrorRetrievingGlossaryDetails() {
    doThrow(new RuntimeException("External Smartling error"))
        .when(mockSmartlingClient)
        .getGlossaryDetails(anyString(), anyString());
    thirdPartyTMSSmartlingGlossary.pullSourceTextUnits(mockRepository, "someUid", localeMapping);
  }
  ...
  public void testPull()
      throws VirutalAssetMissingTextUnitException, VirtualAssetRequiredException {
    thirdPartyTMSSmartlingGlossary.pull(mockRepository, "someUid", localeMapping);
    verify(mockSmartlingClient, times(1))
        .downloadGlossaryFileWithTranslations("testAccountId", "someUid", "fr-FR", "en-US");
    verify(mockVirtualAssetService, times(1))
        .importLocalizedTextUnits(
            assetIdCaptor.capture(), localeIdCaptor.capture(), textUnitsCaptor.capture());
    ...
    assertTrue(
        translatedTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("l'école")
                            && textUnit.getName().equals(SCHOOL_TEXT_UNIT_ID))
                .count()
            == 1);
    assertTrue(
        translatedTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("maison")
                            && textUnit.getName().equals(HOUSE_TEXT_UNIT_ID))
                .count()
            == 1);
    assertTrue(
        translatedTextUnits.stream()
                .filter(
                    textUnit ->
                        textUnit.getContent().equals("le champ")
                            && textUnit.getName().equals(FIELD_TEXT_UNIT_ID))
                .count()
            == 1);
  }
  ...
  public void testGetThirdPartyTextUnits() {
    ...
    assertEquals(
        3,
        thirdPartyTextUnits.stream()
            .filter(textUnit -> textUnit.getAssetPath().equals("Test Glossary"))
            .count());
  }
