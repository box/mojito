...
public class PushPullRunCleanupServiceTest extends ServiceTestBase {
    ...
    @Autowired
    PushPullRunCleanupService pushPullRunCleanupService;
    ...
    public void testCleanOldPushPullData() throws Exception {
        ...
        Repository repository = repositoryService.createRepository(testIdWatcher.getEntityName("repository") + "testCleanOldPushPullData");
        ...
        TMTextUnit tmTextUnit1 = tmService.addTMTextUnit(tm.getId(), asset.getId(), "hello_world 1", "Hello World!",
        ...
        pushRunService.associatePushRunToTextUnitIds(pushRun,
                                                     asset,
                                                     Arrays.asList(tmTextUnit1.getId(), tmTextUnit2.getId()));
        ...
        TMTextUnitVariant tuv1 = tmService.addCurrentTMTextUnitVariant(
                tmTextUnit1.getId(),
                frFR.getId(),
                "le hello_world 1");

        TMTextUnitVariant tuv2 = tmService.addCurrentTMTextUnitVariant(
                tmTextUnit1.getId(),
                frFR.getId(),
                "le hello_world 2");

        pullRunAssetService.replaceTextUnitVariants(pullRunAsset,
                                                    frFR.getId(),
                                                    Arrays.asList(tuv1.getId(), tuv2.getId()));
        List<TMTextUnitVariant> recordedVariants = pullRunTextUnitVariantRepository.findByPullRun(pullRun,
                                                                                                  Pageable.unpaged());
        ...
        Assert.assertEquals(2, pullRunTextUnitVariantRepository.findByPullRun(pullRun, Pageable.unpaged()).size());
        ...
        Assert.assertEquals(0,
                            pushRunService.getPushRunTextUnits(pushRun, PageRequest.of(0, Integer.MAX_VALUE)).size());
        Assert.assertEquals(0, pullRunTextUnitVariantRepository.findByPullRun(pullRun, Pageable.unpaged()).size());
