...
public class AssetServiceTest extends ServiceTestBase {
  ...
  public void
    ...
    assertNotNull(
        "asset should be created with a last successful asset extraction",
  ...
  @Test
  public void testAddOrUpdateAssetAndProcessIfNeededShouldStartExtractionIfNewAsset()
      throws Exception {
    ...
    List<AssetExtraction> assetExtractions = assetExtractionRepository.findByAsset(asset);
    assertEquals(
        "There should be 2 assetExtractions created when adding a new asset",
        2,
  ...
  public void
    ...
    Asset asset =
        addAssetAndWaitUntilDoneProcessing(repository.getId(), newContent, path, null, null);
    ...
    // assertEquals("Content of existing branch should be updated if changed", newContent,
    // assetExtractionByBranch.getAssetExtraction().getAssetContent().getContent());
    assertEquals(
        "Content of existing asset should be updated if changed",
        DigestUtils.md5Hex(newContent),
        assetExtractionByBranch.getAssetExtraction().getContentMd5());
    assertNull(
        "Content of existing asset should be null",
        asset.getLastSuccessfulAssetExtraction().getAssetContent());

    List<AssetExtraction> assetExtractions = assetExtractionRepository.findByAsset(asset);
    assertEquals(
        "There should be 2 assetExtractions created when the adding an asset with changed content",
        2,
        assetExtractions.size());
  }

  private Asset addAssetAndWaitUntilDoneProcessing(
      Long repositoryId, String assetContent, String assetPath, String branchName, Long pushRunId)
      throws Exception {
  ...
  public void
    ...
    assertEquals(
        "There should be 2 assetExtractions created when the adding an asset with initial content",
        2,
        assetExtractions.size());
    ...
    // assetExtractionByBranch.getAssetExtraction().getAssetContent().getContent());
    assertEquals(
        "Content md5 of existing branhc should be updated if changed",
        DigestUtils.md5Hex(newContent),
        assetExtractionByBranch.getAssetExtraction().getContentMd5());
    assertNull(
        "Content of existing asset should be null",
    ...
    assertFalse(
        "The asset extraction process should un-delete the deleted asset",
  ...
  public void testAddOrUpdateAssetAndProcessIfNeededReAddingAsset() throws Exception {
    ...
    assetExtractions = assetExtractionRepository.findByAsset(asset);
    assertEquals(
        "When re-adding an asset (same content as previously deleted), it will get processed as normal",
        2,
        assetExtractions.size());
    assertFalse(
        "The asset extraction process should un-delete the deleted asset",
  ...
  public void testFindAllAssets() throws Exception {
    ...
    Asset asset3 =
        addAssetAndWaitUntilDoneProcessing(repository.getId(), content3, path3, "branch2", null);

    String pathOld = "path/to/old";
    String contentOld = "contentOld";
    Asset oldAssetNotProcessing =
        assetService.createAssetWithContent(repository.getId(), pathOld, contentOld);
    ...
    assetService.deleteAssetsOfBranch(
        Sets.newHashSet(asset2.getId(), asset3.getId()), branchNull.getId());
    all = assetService.findAll(repository.getId(), null, false, null, branchNull.getId());
  ...
  public void testAddAssetWithPushRunShouldCapturePushRunInformation() throws Exception {
    ...
    Asset asset =
        addAssetAndWaitUntilDoneProcessing(
            repository.getId(), content, path, null, pushRun.getId());
  ...
  private PushRunAsset getPushRunAsset(Long pushRunId) {
    ...
    return pushRun.getPushRunAssets().stream().findFirst().orElse(null);
  }

  @Transactional
  private List<TMTextUnit> getTextUnitsFromPushRun(Long pushRunId) {
    PushRun pushRun = pushRunService.getPushRunById(pushRunId);
    PushRunAsset pushRunAsset = pushRun.getPushRunAssets().stream().findFirst().orElse(null);

    return pushRunAsset.getPushRunAssetTmTextUnits().stream()
