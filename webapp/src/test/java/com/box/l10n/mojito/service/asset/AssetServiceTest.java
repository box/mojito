...
public class AssetServiceTest extends ServiceTestBase {
    ...
    public void testAddOrUpdateAssetAndProcessIfNeededShouldStartExtractionIfAssetExistButAssetExtractionIsMissing() throws Exception {
        ...
        assertNotNull("asset should be created with a last successful asset extraction", asset.getLastSuccessfulAssetExtraction());
    ...
    public void testAddOrUpdateAssetAndProcessIfNeededShouldUpdateAssetContentAndStartExtractionIfContentOfExistingAssetChanged() throws Exception {
        ...
        AssetExtractionByBranch assetExtractionByBranch = assetExtractionByBranchRepository.findByAssetAndBranch(asset, branch).get();
        ...
        assertEquals("Content of existing asset should be updated if changed", DigestUtils.md5Hex(newContent), assetExtractionByBranch.getAssetExtraction().getContentMd5());
        assertNull("Content of existing asset should be null", asset.getLastSuccessfulAssetExtraction().getAssetContent());

        List<AssetExtraction> assetExtractions = assetExtractionRepository.findByAsset(asset);
        assertEquals("There should be 2 assetExtractions created when the adding an asset with changed content", 2, assetExtractions.size());
    }
    ...
    private Asset addAssetAndWaitUntilDoneProcessing(Long repositoryId, String assetContent, String assetPath, String branchName) throws Exception {
        ...
        PollableFuture<Asset> assetResult = assetService.addOrUpdateAssetAndProcessIfNeeded(repositoryId, assetPath, assetContent,
                false, branchName, null, null, null);
    ...
    public void testAddOrUpdateAssetAndProcessIfNeededShouldUpdateAssetContentAndStartExtractionIfAssetDeletedAndAssetExtractionIsOutdated() throws Exception {
        ...
        // content is not saved anymore, need to see if we want to restore it or not
        // assertEquals("Content of existing branch should be updated if changed", newContent, assetExtractionByBranch.getAssetExtraction().getAssetContent().getContent());
        assertEquals("Content md5 of existing branhc should be updated if changed", DigestUtils.md5Hex(newContent), assetExtractionByBranch.getAssetExtraction().getContentMd5());
        assertNull("Content of existing asset should be null", asset.getLastSuccessfulAssetExtraction().getAssetContent());
