...
public class TMTextUnitVariantCommentServiceTest extends ServiceTestBase {
    ...
    public void testAddComment() {
        ...
        List<TMTextUnitVariantComment> initialComments = tmTextUnitVariantCommentRepository.findAllByTmTextUnitVariant_id(tmTextUnitVariantId);
        Assert.assertEquals("There should be no comment in the TextUnitVariant", 0L, initialComments.size());

        TMTextUnitVariantComment addComment = tmTextUnitVariantCommentService.addComment(
                tmTextUnitVariantId,
                TMTextUnitVariantComment.Type.LEVERAGING,
                TMTextUnitVariantComment.Severity.INFO,
                "for test");

        List<TMTextUnitVariantComment> comments = tmTextUnitVariantCommentRepository.findAllByTmTextUnitVariant_id(tmTextUnitVariantId);
        Assert.assertEquals("There should be 1 comment in the TextUnitVariant", 1L, comments.size());

        Assert.assertEquals("Comment should be the one just created", addComment.getId(), comments.get(0).getId());
    ...
    public void testCopyComments() {
        ...
        tmTextUnitVariantCommentService.copyComments(sourceTmTextUnitVariantId, targetTmTextUnitVariantId);

        List<TMTextUnitVariantComment> targetComments = tmTextUnitVariantCommentRepository.findAllByTmTextUnitVariant_id(sourceTmTextUnitVariantId);
        Assert.assertEquals("There should be 2 copied comments in the TextUnitVariant", 2L, targetComments.size());

        Iterator<TMTextUnitVariantComment> sourceCommentsIt = tmTextUnitVariantCommentRepository.findAllByTmTextUnitVariant_id(targetTmTextUnitVariantId).iterator();

        for (TMTextUnitVariantComment targetComment : targetComments) {
            TMTextUnitVariantComment sourceComment = sourceCommentsIt.next();

            Assert.assertNotEquals("Ids should be different", sourceComment.getId(), targetComment.getId());
    ...
    public void testEnrichTextUnitDTOWithComments() {
        ...
        List<TextUnitDTOWithComments> textUnitDTOsWithComments = tmTextUnitVariantCommentService.enrichTextUnitDTOsWithComments(textUnitDTOs);
        Assert.assertEquals("There should be only one DTO with comments returned", 1, textUnitDTOsWithComments.size());

        List<TMTextUnitVariantComment> variantComments = textUnitDTOsWithComments.get(0).getTmTextUnitVariantComments();
        ...
        Assert.assertEquals(TMTextUnitVariantComment.Severity.INFO, variantComments.get(0).getSeverity());
