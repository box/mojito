...
public class ThirdPartyServiceTestData {
  ...
  public ThirdPartyServiceTestData init() throws Exception {
    ...
    byte[] content2 =
        ByteStreams.toByteArray(
            new ClassPathResource("/com/box/l10n/mojito/img/2.png").getInputStream());
    ...
    tmTextUnitHello =
        tmTextUnits.stream().filter(t -> "hello".equals(t.getName())).findFirst().get();
    tmTextUnitBye = tmTextUnits.stream().filter(t -> "bye".equals(t.getName())).findFirst().get();
    tmTextUnitPluralThingsZero =
        tmTextUnits.stream()
            .filter(t -> "plural_things_zero".equals(t.getName()))
            .findFirst()
            .get();
    tmTextUnitPluralThingsOne =
        tmTextUnits.stream().filter(t -> "plural_things_one".equals(t.getName())).findFirst().get();
    tmTextUnitPluralThingsTwo =
        tmTextUnits.stream().filter(t -> "plural_things_two".equals(t.getName())).findFirst().get();
    tmTextUnitPluralThingsFew =
        tmTextUnits.stream().filter(t -> "plural_things_few".equals(t.getName())).findFirst().get();
    tmTextUnitPluralThingsMany =
        tmTextUnits.stream()
            .filter(t -> "plural_things_many".equals(t.getName()))
            .findFirst()
            .get();
    tmTextUnitPluralThingsOther =
        tmTextUnits.stream()
            .filter(t -> "plural_things_other".equals(t.getName()))
            .findFirst()
            .get();
