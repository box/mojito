...
6. **Export a vendor drop** — **`mojito_drop_export`** with `repositoryId` and BCP-47 `locales` (from **`mojito_repo_view`**). Optional `type` (`TRANSLATION` default / `REVIEW`) and `useInheritance` (REVIEW only). Unlike CLI `drop-export`, omitting `locales` does not fill repository locales. The response has `dropId` and `pollableTask`; **poll `pollableTask.id` with `mojito_pollabletask_get`** until finished. This tool does not wait. Confirm environment first.
7. **Import a vendor drop** — **`mojito_drop_import`** with `repositoryId` and `dropId`. Optional `status` (`APPROVED` / `REVIEW_NEEDED` / `TRANSLATION_NEEDED`). **Poll the returned `pollableTask.id` with `mojito_pollabletask_get`**. This tool does not wait. Confirm environment first. List/cancel/complete/xliff-import are not available.
8. **Find strings or translations** — **`mojito_textunit_search`** with `repositoryNames` or `repositoryIds` plus optional `source`, `name` (string id), `target`, `assetPath`, `branchId`, `statusFilter`, `localeTags`, `searchType` (`EXACT` / `CONTAINS` / `ILIKE`), `usedFilter`, date range, `limit`. Omit `repositoryIds`/`repositoryNames` for all repos; omit `localeTags` for all locales. Prefer scoping by repo — all-repo search can be large.
9. **Inspect one unit** — **`mojito_textunit_info`** with `tmTextUnitId` (optional `localeTags`) for created date, status, current translation, asset path, used/DNT, etc.
10. **Explain changes over time** — **`mojito_textunit_history`** with `tmTextUnitId` and **`bcp47Tag`** (required; e.g. `fr-FR`, not locale id).
11. **Submit a translation** — **`mojito_textunit_translation_add`** with `tmTextUnitId`, **numeric `localeId`** (from search/info, not the BCP-47 tag), and `target`. Optional `targetComment`, `status`, `includedInLocalizedFile`.
12. **Workbench review** — **`mojito_review_update`** with `tmTextUnitId`, `localeId`, current `target`, and `action`: `accept` / `review` / `translate` / `reject`. Prefer this over raw status fields for accept/reject/needs-review flows.
13. **Create/delete a repository** — **`mojito_repo_create`** / **`mojito_repo_delete`**. Confirm environment and id/name with the user first, especially on prod.
14. **Delete a source asset** — **`mojito_asset_delete`** with `assetId`. Confirm environment and id first; this is a single-asset delete, not CLI push cleanup.
15. **Async jobs** — **`mojito_pollabletask_get`** with a pollable task id. This tool does **not** wait; poll until finished (backoff). Use it after **`mojito_drop_export`**, **`mojito_drop_import`**, and **`mojito_asset_import`**. v1 tools do not auto-wait on CLI `--wait`.
