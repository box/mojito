...
public class ReswFileTypeTest {
    ...
    public void testTargetPattern() {
        ReswFileType reswFileType = new ReswFileType();
        Matcher matcher = reswFileType.getTargetFilePattern().getPattern().matcher("/source/fr/Resources.resw");
        Assert.assertTrue(matcher.matches());
        Assert.assertEquals("/source/", matcher.group(PARENT_PATH));
    ...
    public void testTargetPatternBcp47() {
        ReswFileType reswFileType = new ReswFileType();
        Matcher matcher = reswFileType.getTargetFilePattern().getPattern().matcher("/source/en-GB/Resources.resw");
        Assert.assertTrue(matcher.matches());
        Assert.assertEquals("/source/", matcher.group(PARENT_PATH));
    ...
    public void testTargetPatternNoMatch() {
        ReswFileType reswFileType = new ReswFileType();
        Matcher matcher = reswFileType.getTargetFilePattern().getPattern().matcher("/source/en/Resources.resw");
        Assert.assertFalse(matcher.matches());
    }
