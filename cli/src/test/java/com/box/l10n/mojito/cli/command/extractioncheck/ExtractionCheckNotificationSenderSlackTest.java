...
public class ExtractionCheckNotificationSenderSlackTest  {
    ...
    @MockBean
    SlackChannels slackChannelsMock;
    ...
    @Before
    public void setup() {
        extractionCheckNotificationSenderSlack = new ExtractionCheckNotificationSenderSlack("user",
                "{0}@somewhere.com", "{baseMessage}", "This is a hard failure message.",
                "This is a checks skipped message.", false);
    ...
    public void testNoNotificationsSentIfNoFailures() throws SlackClientException {
        ...
        verify(slackChannelsMock, times(0)).getSlackChannelForDirectOrBotMessage(isA(Boolean.class), isA(String.class), isA(String.class));
    }
    ...
    public void testExceptionThrownIfSlackClientFails() throws SlackClientException {
        when(slackClientMock.sendInstantMessage(isA(Message.class))).thenThrow(SlackClientException.class);
        List<CliCheckResult> results = new ArrayList<>();
        CliCheckResult result = new CliCheckResult(false, false, "Test Check");
        result.setNotificationText("Some notification text");
        results.add(result);
