...
public class LeveragingCommandTest extends CLITestBase {
    ...
    public void copyTMModeTargetBranchName() throws Exception {
        ...
        getL10nJCommander().run("push", "-r", targetRepository.getName(),
                "-s", getInputResourcesTestDir("source2").getAbsolutePath(), "-b", "branch1");

        getL10nJCommander().run("push", "-r", targetRepository.getName(),
                "-s", getInputResourcesTestDir("source2b").getAbsolutePath());
        ...
        getL10nJCommander().run("leveraging-copy-tm", "-m", "MD5", "-s", sourceRepository.getName(), "-t", targetRepository.getName(), "--target-branch-name", "branch1");


        List<String> targetTranslations = tmTextUnitVariantRepository.findByTmTextUnitTmRepositoriesOrderByContent(targetRepository)
                .stream()
                .sorted(Comparator.comparing(TMTextUnitVariant::getLocale, Comparator.comparing(Locale::getBcp47Tag))
                        .thenComparing(TMTextUnitVariant::getContent))
                .map(TMTextUnitVariant::getContent)
                .peek(t -> logger.info("target translation: {}", t))
                .collect(Collectors.toList());

        List<String> expectedTargetTranslations = Arrays.asList(
                "1 day", "1 hour", "1 month", "100 character description:", "15 min",
                "1 jour", "Description de 100 caractères :",
                "100文字の説明：", "1日"
        );

        Assert.assertEquals("translation in source and target must be the same",
                expectedTargetTranslations, targetTranslations);
    }
