...
public class ExtractionDiffPathsTest extends IOTestBase {
  ...
  public void extractionDiffPath() {
    ExtractionDiffPaths extractionDiffPaths = getExtractionDiffsPaths();
    Assert.assertEquals(
        Paths.get(outputDirectory, extractionDiffName), extractionDiffPaths.extractionDiffPath());
  }
  ...
  public void sourceFileMatchPath() {
    ExtractionDiffPaths extractionDiffPaths = getExtractionDiffsPaths();
    extractionDiffPaths.sourceFileMatchPath(
        Paths.get(outputDirectory, extractionDiffName, "LC_MESSAGES/messages.pot.json"));
  }
  ...
  public void findAllAssetExtractionDiffPaths() {
    ...
    Set<Path> allAssetExtractionPaths =
        extractionDiffPaths.findAllAssetExtractionDiffPaths().collect(Collectors.toSet());
    Assert.assertEquals(
        Sets.newHashSet(
            inputResourcesTestDir
                .toPath()
                .resolve(Paths.get("testExtractionDiffName/LC_MESSAGES/messages.pot.json")),
            inputResourcesTestDir
                .toPath()
                .resolve(Paths.get("testExtractionDiffName/LC_MESSAGES/messages2.pot.json"))),
