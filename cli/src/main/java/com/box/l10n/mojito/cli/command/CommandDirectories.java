...
/**
 * Class to manage source and target directories used by {@link Command} and provides utilities to
 * manipulate files within those directories.
 *
 * @author jaurambault
 */
public class CommandDirectories {
  ...
  /**
   * Creates the instance from path string of the source directory (the source directory will be
   * used as target directory).
   *
   * @param sourceDirectoryPathString source directory, if {@code null} the current user directory
   *     will be used
   */
  ...
  /**
   * Creates the instance from path strings of the source and target directories.
   *
   * @param sourceDirectoryPathString source directory, if {@code null} the current user directory
   *     will be used
   * @param targetDirectoryPathString target directory, if {@code null} the source directory will be
   *     used
   */
  public CommandDirectories(String sourceDirectoryPathString, String targetDirectoryPathString) {

    sourceDirectoryPathString =
        MoreObjects.firstNonNull(sourceDirectoryPathString, System.getProperty("user.dir"));
    targetDirectoryPathString =
        MoreObjects.firstNonNull(targetDirectoryPathString, sourceDirectoryPathString);
  ...
  /**
   * Gets a path that has the same relative path in the target directory as the given path in the
   * source directory.
   *
   * <p>With source directory: {@code "source"}, target directory: {@code "target"} and
   * pathInSourceDirectory: {@code "source/sub/file1"} will return {@code "target/sub/file1"}
   *
  ...
  public Path resolveWithTargetDirectory(Path pathInSourceDirectory) {
    return relativizeWithUserDirectory(
        targetDirectoryPath.resolve(relativizeWithSourceDirectory(pathInSourceDirectory)));
  }

  /**
   * Gets a path that has the same relative path in the target directory as the given path in the
   * source directory and creates parent directories of the target path if they don't exist.
   *
   * <p>With source directory: {@code "source"}, target directory: {@code "target"} and
   * pathInSourceDirectory: {@code "source/sub/file1"} will return {@code "target/sub/file1"}.
   * {@code "target/sub/"} will be created.
   *
   * @param pathInSourceDirectory path relativized in source directory
   * @return path in the target directory
   */
  public Path resolveWithTargetDirectoryAndCreateParentDirectories(Path pathInSourceDirectory)
   ...
   * @throws CommandException
   */
  public List<Path> listFilesWithExtensionInSourceDirectory(String... extensions)
      throws CommandException {

    final List<Path> paths = new ArrayList<>();

    final PathMatcher pathMatcher =
        FileSystems.getDefault().getPathMatcher(buildGlobPatternForFileWithExtensions(extensions));

    SimpleFileVisitor<Path> extensionFileVisitor =
    ...
    try {
      ...
      throw new CommandException(
          "Error while listing the file for extension: " + Arrays.toString(extensions), ioe);
    }
  ...
  /**
   * Build a Glob pattern that matches files with given extensions (case insensitive) across
   * boundaries: glob:**.{ext1,EXT1,ext2,EXT2 ...}.
   *
