...
/**
 * {@link AbstractCliChecker} that uses hunspell native libraries to spell check words in source
 * strings. <br>
 * <br>
 * <b>NOTE:</b> The hunspell libraries must be available on your system separately for this check to
...
public class SpellCliChecker extends AbstractCliChecker {
  ...
  static class SpellCliCheckerResult {
    ...
    public boolean equals(Object o) {
      ...
      return isSuccessful == that.isSuccessful
          ...
          && Objects.equals(suggestionMap, that.suggestionMap);
    }
  ...
  protected Hunspell getHunspellInstance() {
    if (hunspell == null) {
      hunspell =
          new Hunspell(
              Paths.get(cliCheckerOptions.getDictionaryFilePath()),
              Paths.get(cliCheckerOptions.getDictionaryAffixFilePath()));
    }
    return hunspell;
  }
  ...
  private void checkDictionaryFiles() {
    if (StringUtils.isBlank(cliCheckerOptions.getDictionaryFilePath())) {
      throw new CommandException(
          "The dictionary file path parameter cannot be empty when using the spell checker.");
    } else if (StringUtils.isBlank(cliCheckerOptions.getDictionaryAffixFilePath())) {
      throw new CommandException(
          "The dictionary affix file path parameter cannot be empty when using the spell checker.");
      ...
      throw new CommandException(
          cliCheckerOptions.getDictionaryAffixFilePath() + " does not exist.");
    }
  }
  ...
  private Map<String, Map<String, List<String>>> spellCheck(List<String> sourceStrings) {
    ...
    return sourceStrings.stream()
        ...
        .collect(
            Collectors.toMap(
                SpellCliCheckerResult::getSource, SpellCliCheckerResult::getSuggestionMap));
  }

  private SpellCliCheckerResult getSpellCliCheckerResult(String sourceString) {
    SpellCliCheckerResult result =
  ...
  private void loadAdditionalWordsToDictionary(String additionalWordsFilePath) {
    ...
    if (additionalWordsFilePath != null && !additionalWordsFilePath.isEmpty()) {
      try {
        Files.readAllLines(Paths.get(additionalWordsFilePath)).stream()
            .forEach(
                word -> {
                  String w = word.trim();
        ...
        throw new CommandException(
            "Error adding additional words to dictionary: " + e.getMessage());
      }
    }
  }
  ...
  private Map<String, List<String>> spellCheckSourceString(List<String> words) {
    ...
    for (String word : words) {
      if (!checked.contains(word) && !hunspell.spell(word)) {
        ...
        logger.debug(
            "{} is spelt incorrectly. Suggested correct spellings are {}", word, suggestions);
  ...
  private void addDictionaryUpdateInformation(StringBuilder notificationText) {
    if (cliCheckerOptions.getDictionaryAdditionsFilePath() != null
        && !cliCheckerOptions.getDictionaryAdditionsFilePath().isEmpty()) {
      notificationText.append(
          "If a word is correctly spelt please add your spelling to "
  ...
  private static Function<List<String>, String> joinCommaSeparated(
      String delimiter, String finalDelimiter) {
    return result -> {
      ...
      return String.join(
          finalDelimiter, String.join(delimiter, result.subList(0, last)), result.get(last));
    };
  }
}
