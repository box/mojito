...
public class DropCompleteCommand extends Command {
    ...
    @Parameter(names = {"--drop-id", "-i"}, arity = 1, required = false, description = "ID of a drop to mark as complete (skip drop fetching and process only given ID)")
    Long dropId = null;
    ...
    private void completeDropsSelectingFromAvailableList() throws CommandException {
        ...
        if (numberedAvailableDrops.isEmpty()) {
            ...
            for(Long dropId: dropIds) {
                ...
                // TODO make drop complete a pollable task to get come confirmation that the action has been performed
                dropClient.completeDrop(dropId);
            }
        }
    }
