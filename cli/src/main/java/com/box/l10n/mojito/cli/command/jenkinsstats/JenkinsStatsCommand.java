...
@Component
@Scope("prototype")
@Parameters(commandNames = {"jenkins-stats"}, commandDescription = "Compute basic statistics (success rate, and average duration) for Jenkins jobs")
public class JenkinsStatsCommand extends Command {

    @Parameter(names = "--config", required = true, description = "Path to configuration file. Format: " +
    ...
    protected void execute() throws CommandException {
        ...
        printSuccessRate(jobResults, now, nowMinus1Day, yesterdayEnd, yesterdayStart, weekEnd, weekStart);
        printAvgTimes(jobResults, now, nowMinus1Day, yesterdayEnd, yesterdayStart, weekEnd, weekStart);
    }

    void printAvgTimes(ImmutableList<JenkinsJobResult> jobResults, Instant now, Instant nowMinus1Day, Instant yesterdayEnd, Instant yesterdayStart, Instant weekEnd, Instant weekStart) {
        printAvgTimeHeader();
        jobResults.stream()
                .collect(Collectors.groupingBy(JenkinsJobResult::getJobName))
                .forEach((jobName, jenkinsJobResults) -> {
                    long avgCurrentDay = jenkinsJobResults.stream()
                            .filter(jenkinsJobResult -> nowMinus1Day.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < now.toEpochMilli())
                            .collect(Collectors.averagingLong(JenkinsJobResult::getDuration)).longValue();

                    long avgYesterday = jenkinsJobResults.stream()
                            .filter(jenkinsJobResult -> yesterdayStart.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < yesterdayEnd.toEpochMilli())
                            .collect(Collectors.averagingLong(JenkinsJobResult::getDuration)).longValue();

                    long avgWeek = jenkinsJobResults.stream()
                            .filter(jenkinsJobResult -> weekStart.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < weekEnd.toEpochMilli())
                            .collect(Collectors.averagingLong(JenkinsJobResult::getDuration)).longValue();

                    long avgGlobal = jenkinsJobResults.stream()
                            .collect(Collectors.averagingLong(JenkinsJobResult::getDuration)).longValue();
                    printAvgTimes(jobName, avgCurrentDay, avgYesterday, avgWeek, avgGlobal);
                });
    }

    void printSuccessRate(ImmutableList<JenkinsJobResult> jobResults, Instant now, Instant nowMinus1Day, Instant yesterdayEnd, Instant yesterdayStart, Instant weekEnd, Instant weekStart) {
        printSuccessRateHeader();
        printSuccessRate("global", jobResults.stream()
                .collect(Collectors.groupingBy(o -> o.getResult(), Collectors.counting())));
        printSuccessRate("today", jobResults.stream()
                .filter(jenkinsJobResult -> nowMinus1Day.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < now.toEpochMilli())
                .collect(Collectors.groupingBy(o -> o.getResult(), Collectors.counting())));

        printSuccessRate("yesterday", jobResults.stream()
                .filter(jenkinsJobResult -> yesterdayStart.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < yesterdayEnd.toEpochMilli())
                .collect(Collectors.groupingBy(o -> o.getResult(), Collectors.counting())));

        printSuccessRate("last week", jobResults.stream()
                .filter(jenkinsJobResult -> weekStart.toEpochMilli() < jenkinsJobResult.getTimestamp() && jenkinsJobResult.getTimestamp() < weekEnd.toEpochMilli())
                .collect(Collectors.groupingBy(o -> o.getResult(), Collectors.counting())));

        jobResults.stream().collect(Collectors.groupingBy(JenkinsJobResult::getJobName))
                .forEach((jobName, jenkinsJobResults) -> {
                    Map<String, Long> counts = jenkinsJobResults.stream()
                            .collect(Collectors.groupingBy(o -> o.getResult(), Collectors.counting()));
     ...
     */
    void printAvgTimes(String jobName, long avgCurrentDay, long avgYesterday, long avgWeek, long avgGlobal) {
        ...
        consoleWriter.a(jobName + "," + avgCurrentDay + "," + avgYesterday + "," + avgWeek + "," + avgGlobal).println();
    }
    ...
    void printSuccessRate(String name, Map<String, Long> resultCounts) {
        ...
        consoleWriter.a(name + "," + success + "," + failure + "," + (double) success / (success + failure)).println();
    }

    ImmutableList<JenkinsJobResult> getJobResults(List<String> jobUrls) {
        return jobUrls.stream()
                .map(this::getJobResults)
                .flatMap(jenkinsJobResults -> jenkinsJobResults.getBuilds().stream()
                        .map(jenkinsJobResult -> jenkinsJobResult.withJobName(jenkinsJobResults.getDisplayName())))
    ...
    JenkinsJobResults getJobResults(String baseJobUrl) {
        try {
            HttpURLConnection httpURLConnection = (HttpURLConnection) new URL(baseJobUrl +
            ...
            JenkinsJobResults jenkinsJobResults = mapper.readValue(responseStream, JenkinsJobResults.class);
    ...
    void initFromConfigurationFile() {
        try {
            jenkinsStatsConfig = mapper.readValueUnchecked(Paths.get(config).toFile(), JenkinsStatsConfig.class);
        } catch (Exception e) {
            throw new CommandException("Invalid config file, format: \n" +
