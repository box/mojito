...
public abstract class ExtractionCheckNotificationSender {
    ...
    public ExtractionCheckNotificationSender(String messageTemplate, String hardFailureMessage, String checksSkippedMessage) {
    ...
    protected Stream<CliCheckResult> getCheckerHardFailures(List<CliCheckResult> results) {
        return results.stream().filter(result -> !result.isSuccessful()).filter(CliCheckResult::isHardFail);
    }

    protected String getFormattedNotificationMessage(String messageTemplate, String messageKey, String message) {
    ...
    protected String appendHardFailureMessage(boolean hardFail, StringBuilder sb) {
        return hardFail && !Strings.isNullOrEmpty(hardFailureMessage) ? sb + getDoubleNewLines() + hardFailureMessage : sb.toString();
    }
