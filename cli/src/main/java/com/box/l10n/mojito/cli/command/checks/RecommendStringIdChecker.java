...
/**
 * {@link AbstractCliChecker} that generates a recommended string id based off the file path of the
 * file that contains the new string.
 *
 * @author mallen
 */
public class RecommendStringIdChecker extends AbstractCliChecker {
  ...
  private String removeLineNumberFromUsage(String usage) {
    ...
    if (lastIndexOfColon > -1
        && lastIndexOfColon < usage.length() - 1
        && StringUtils.isNumeric(usage.substring(lastIndexOfColon + 1))) {
      usage = usage.substring(0, lastIndexOfColon);
    }
    return usage;
  }

  private RecommendStringIdCheckResult generateRecommendation(
      AssetExtractorTextUnit textUnit, RecommendStringIdCheckResult recommendation) {
    List<String> possiblePrefixes = new ArrayList<>();
    String msgctxt =
        removeIgnoredLabelFromContext(
    ...
    if (!possiblePrefixes.isEmpty()
        && !possiblePrefixes.stream().anyMatch(prefix -> msgctxt.startsWith(prefix))) {
