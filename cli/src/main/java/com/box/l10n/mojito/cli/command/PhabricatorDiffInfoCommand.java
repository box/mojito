 ...
 *
 * <p>It will export revision id, base commit, author email and author username (that's computed by
 * removing the email part from the author email, it is not coming from phabricator).
 *
 * <p>Format is a list of environment variables like this: MOJITO_PHAB_REVISION_ID=789456
 * MOJITO_PHAB_BASE_COMMIT=c32dadb23 MOJITO_PHAB_AUTHOR_EMAIL=user@test.com
 * MOJITO_PHAB_AUTHOR_USERNAME=user
 *
 * <p>Example how to source the result of the command in bash shell source <(mojito
 * phab-diff-to-env-variables --diff-id 123456)
 */
@Component
@Scope("prototype")
@Parameters(
    commandNames = {"phab-diff-to-env-variables"},
    commandDescription =
        "Get diff info: revision id, base commit, "
...
public class PhabricatorDiffInfoCommand extends Command {
  ...
  public void execute() throws CommandException {
    ...
    consoleWriterAnsiCodeEnabledFalse
        .a("MOJITO_PHAB_REVISION_ID=")
        .a(queryDiffsFields.getRevisionId())
        .println();
    consoleWriterAnsiCodeEnabledFalse
        .a("MOJITO_PHAB_BASE_COMMIT=")
        .a(queryDiffsFields.getSourceControlBaseRevision())
        .println();
    consoleWriterAnsiCodeEnabledFalse
        .a("MOJITO_PHAB_AUTHOR_EMAIL=")
        .a(queryDiffsFields.getAuthorEmail())
        .println();
    consoleWriterAnsiCodeEnabledFalse
        .a("MOJITO_PHAB_AUTHOR_USERNAME=")
        .a(getUsernameForAuthorEmail(queryDiffsFields.getAuthorEmail()))
        .println();

    if (StringUtils.isNotBlank(queryDiffsFields.getRevisionId())
        && differentialRevision
            .getTestPlan(queryDiffsFields.getRevisionId())
            .contains(SKIP_I18N_CHECKS_FLAG)) {
