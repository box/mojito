...
/**
 * Checks for placeholders by number using a regex pattern, expects to find descriptions in the
 * comment that matches the number of placeholders in the source string.
 *
 * @author mallen
 */
public class SimpleRegexPlaceholderDescriptionChecker extends AbstractPlaceholderDescriptionCheck {
  ...
  public SimpleRegexPlaceholderDescriptionChecker(
      PlaceholderRegularExpressions placeholderRegularExpressions) {
  ...
  protected List<String> getPlaceholderNames(String source, Matcher placeHolderMatcher) {
    ...
    while (placeHolderMatcher.find()) {
      String placeholder =
          source
              .substring(placeHolderMatcher.start(), placeHolderMatcher.end())
              .replaceAll("\\[\\d+\\]", "");
