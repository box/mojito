...
public class GlossaryTermCaseCheckerTrie {
  ...
  private boolean isGlossaryTermValid(String source, Node current) {
    ...
    for (GlossaryTerm glossaryTerm : current.glossaryTerms) {
      ...
      if (index > -1) {
        String glossaryTermInString =
            source.substring(index, index + glossaryTerm.getTerm().length());
  ...
  private void addFailureText(
      String source, GlossaryCaseCheckerSearchResult result, List<String> failures, Node current) {
    result.isSuccess = false;
    if (current.glossaryTerms.stream()
        .anyMatch(term -> term.getSeverity() == GlossaryTermSeverity.MAJOR)) {
      ...
      failures.add(
          String.format(
              "MAJOR: String "
                  ...
                  + "%s"
                  + QUOTE_MARKER
                  + " contains glossary term '%s' which must match exactly.",
              source,
              current.glossaryTerms.stream()
                  .filter(term -> term.getSeverity() == GlossaryTermSeverity.MAJOR)
        ...
        failures.add(
            String.format(
                "WARN: String "
                    ...
                    + "%s"
                    + QUOTE_MARKER
                    + " contains glossary terms %s but does not exactly match one of the terms.",
                source,
                current.glossaryTerms.stream()
        ...
        failures.add(
            String.format(
                "WARN: String "
                    ...
                    + "%s"
                    + QUOTE_MARKER
                    + " contains glossary term '%s' but does not exactly match the glossary term.",
                source,
                current.glossaryTerms.get(0).getTerm()));
      }
    }
  }
