...
/**
 * Contains logic to generate paths for local extractions and convert them back to original
 * filenames
 */
public class ExtractionPaths {
  ...
  Set<Path> findAllAssetExtractionPaths() {
    return Files.find(extractionPath(), 100, (p, f) -> p.toString().endsWith(JSON_FILE_EXTENSION))
