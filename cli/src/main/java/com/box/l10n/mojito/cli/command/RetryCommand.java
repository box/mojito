...
/**
 * Command to execute shell command with retry and exponential back off. Can optional tell in which
 * directory the command should be executed.
 *
 * @author jaurambault
 */
@Component
...
@Parameters(
    commandNames = {"retry"},
    commandDescription = "Execute a shell command with retry")
public class RetryCommand extends Command {
  ...
  @Parameter(
      names = {"-i", "--initial-interval"},
      arity = 1,
      required = false,
      description = "Initial interval for the back off policy in milliseconds")
  ...
  @Parameter(
      ...
      arity = 1,
      required = false,
      description = "Multiplier interval for the back off policy")
  double multiplier = 1.5;

  @Parameter(
      names = {COMMAND_SHORT, COMMAND_LONG},
      required = true,
      description =
  ...
  @Parameter List<String> parameters;
  ...
  public boolean shouldShowUsage() {
    ...
    return (indexOfHelpParameter >= 0
        && (indexOfHelpParameter < indexOfCommandParameter || indexOfCommandParameter == -1));
  }
  ...
  void showUsage() {
    if (getIndexOfCommandParameter() == -1) {
      consoleWriter
          .newLine()
          .fg(Ansi.Color.RED)
          .a("The following option is required: ")
          .a(COMMAND_SHORT)
          .a(",")
          .a(COMMAND_LONG)
          .println(2);
  ...
  List<String> getCommandToExecute() {
    ...
    List<String> commandToExecute =
        originalArgs.subList(indexOfCommandParameter + 1, originalArgs.size());
