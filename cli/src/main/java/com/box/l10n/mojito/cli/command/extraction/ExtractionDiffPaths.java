...
public class ExtractionDiffPaths {
  ...
  private ExtractionDiffPaths(
      Path outputDirectory,
      String diffExtractionName,
      ExtractionPaths baseExtractionPaths,
      ExtractionPaths currentExtractionPath) {
  ...
  public String sourceFileMatchPath(Path extractionDiffPath) {
    ...
    String withoutExtension =
        relativePath.substring(0, relativePath.length() - JSON_FILE_EXTENSION.length());
    return withoutExtension;
  }

  public Stream<Path> findAllAssetExtractionDiffPaths() {
    return Files.find(
        extractionDiffPath(), 100, (p, f) -> p.toString().endsWith(JSON_FILE_EXTENSION));
  }
  ...
  public static class Builder {
    ...
    public ExtractionDiffPaths build() {
      ...
      if (diffExtractionName == null) {
        diffExtractionName =
            currentExtractorPaths.getExtractionName()
                + "_"
                + baseExtractorPaths.getExtractionName();
      }

      return new ExtractionDiffPaths(
          Paths.get(outputDirectory),
          diffExtractionName,
          baseExtractorPaths,
          currentExtractorPaths);
    }
  }
}
