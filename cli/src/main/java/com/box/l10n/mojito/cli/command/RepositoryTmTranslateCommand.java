...
/**
 * Command to populate untranslated string with translation from another repository translation memory.
 * This meant to process small volumes but can be helpful to combine the with the import localized files command.
 * <p>
 * Matches the text units by the source. Use text unit name and comment match as priority (moved assets), else just take the
 * newest translation match. Has an option to not import string that have same source and target to avoid
 * copy translation that might be from importing partly translated files. The drawback is that it could skip
 * string that should be imported but the benefits is that the localized files will be same regardless.
 * <p>
...
@Parameters(
        ...
        commandDescription = "Translate a repository using another repository TM. Only untranslated text units" +
...
public class RepositoryTmTranslateCommand extends Command {
    ...
    public void execute() throws CommandException {
        ...
        if (locales == null) {
            locales = targetRepository.getRepositoryLocales().stream().filter(rl -> rl.getParentLocale() != null).map(RepositoryLocale::getLocale).map(Locale::getBcp47Tag).toList();
        }
        ...
        for (String locale : locales) {
            ...
            List<TextUnitClient.TextUnit> untranslatedTextUnitsForLocale = getUntranslatedTextUnitsForLocale(targetRepository.getName(), locale);
            ...
            for (TextUnitClient.TextUnit textUnit : untranslatedTextUnitsForLocale) {
                ...
                List<TextUnitClient.TextUnit> existingTranslationWithSameSource = getTranslationsWithExactSource(sourceRepository.getName(), locale, textUnit.source());

                Optional<TextUnitClient.TextUnit> match = getMatchByNameAndComment(textUnit.source(), textUnit.comment(), existingTranslationWithSameSource);
                if (match.isPresent()) {
            ...
            consoleWriter.a("Saving translations for locale: ").fg(Color.MAGENTA).a(locale).reset().a("Text unit count: ").fg(Color.CYAN).a(textUnitsToSave.size()).println();
            TextUnitClient.ImportTextUnitsBatch importTextUnitsBatch = new TextUnitClient.ImportTextUnitsBatch(false, true, textUnitsToSave);
    ...
    private List<TextUnitClient.TextUnit> getTranslationsWithExactSource(String repositoryName, String locale, String source) {
    ...
    private List<TextUnitClient.TextUnit> getUntranslatedTextUnitsForLocale(String repositoryName, String locale) {
        TextUnitClient.TextUnitSearchBody textUnitSearchBody = new TextUnitClient.TextUnitSearchBody();
        textUnitSearchBody.setRepositoryNames(List.of(repositoryName));
        textUnitSearchBody.setLocaleTags(List.of(locale));
    ...
    private Optional<TextUnitClient.TextUnit> getMatchByNewest(List<TextUnitClient.TextUnit> candidates) {
        return candidates.stream().max(Comparator.comparingLong(TextUnitClient.TextUnit::createdDate));
    }

    private static Optional<TextUnitClient.TextUnit> getMatchByNameAndComment(String name, String comment, List<TextUnitClient.TextUnit> candidates) {
        return candidates.stream().filter(m -> Objects.equals(name, m.name()) && Objects.equals(comment, m.comment())).max(Comparator.comparingLong(TextUnitClient.TextUnit::createdDate));
    }
}
