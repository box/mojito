...
/**
 * Wrapper around {@link java.io.Console} (helpful for for mocking) and provides helpers.
 *
 * @author jaurambault
 */
@Component
public class Console {
   ...
   *
   * @return A string containing the line read from the console, not including any line-termination
   *     characters, or <tt>null</tt> if an end of stream has been reached.
   * @throws CommandException if system console is not available
   */
  ...
  /**
   * Reads a line from the system console, casts it to specified type and returned the casted value.
   *
   ...
   * @throws CommandException if it was not possible to read the input or cast it to specified type.
   */
  public <T> T readLine(Class<T> clazz) throws CommandException {
    ...
    try {
      ...
      throw new RuntimeException(
          "Type ("
              + clazz.getName()
              + ") must implement valueOf with single argument of type String",
          nsme);
    }
