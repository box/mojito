...
@Component
@Scope("prototype")
@Parameters(commandNames = {"tm-import"}, commandDescription = "Import assets into the TMS")
public class TMImportCommand extends Command {
    ...
    @Parameter(names = {"--origin-override"}, arity = 1, required = false, description = "To override the origin attribute in the XLIFF")
    String originOverrideParam;

    @Parameter(names = {"--update-tm", "-u"}, required = false, description = "To update the TM. By default the import assumes that the TM is empty (optimized for faster import)")
    Boolean updateTMParam = false;

    @Parameter(names = {"--skip-source"}, required = false, description = "Skip the source file import")
    ...
    public void execute() throws CommandException {
        ...
        consoleWriter.newLine().a("Start importing for TM: ").fg(Ansi.Color.CYAN).a(repositoryParam).println(2);
    ...
    protected void doImportFileMatch(FileMatch fileMatch) throws CommandException {
        try {
            ...
            consoleWriter.a(" - Importing file: ").fg(Ansi.Color.MAGENTA).a(relativeFilePath.toString()).fg(Ansi.Color.YELLOW).a(" Running").println();

            repositoryClient.importRepository(repository.getId(), getFileContent(fileMatch), updateTMParam);

            consoleWriter.erasePreviouslyPrintedLines();
            consoleWriter.a(" - Importing file: ").fg(Ansi.Color.MAGENTA).a(relativeFilePath.toString()).fg(Ansi.Color.GREEN).a(" Done").println();

        } catch (ResourceNotCreatedException e) {
            throw new CommandException("Error importing [" + fileMatch.getPath().toString() + "] into repo [" + repositoryParam + "]", e);
        }
    }
    ...
    private String getFileContent(FileMatch fileMatch) throws CommandException {
        ...
        if (originOverrideParam != null) {
            fileContent = fileContent.replaceFirst("file.*?original=\"(.*?)\"", "file original=\"" + originOverrideParam + "\"");
        }
