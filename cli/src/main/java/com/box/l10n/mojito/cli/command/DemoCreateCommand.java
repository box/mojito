...
@Component
@Scope("prototype")
@Parameters(commandNames = {"demo-create"}, commandDescription = "Creates a demo repository and local resource bundle")
public class DemoCreateCommand extends RepoCommand {
    ...
    @Parameter(names = {"--output-directory", "-o"}, arity = 1, required = false, description = "Provide the name of the directory that will contain the resource bundle")
    ...
    void createDemoRepository() throws CommandException {
        ...
        consoleWriter.newLine().a("Create demo repository: ").fg(Ansi.Color.CYAN).a(nameParam).println();
    ...
    void push() {
        new L10nJCommander().run("push", "-r", repository.getName(), "-s", outputDirectoryPath.toString());
    }

    URL getResourceURL(String filename) {
        return getClass().getResource("DemoCreateCommand/" + filename);
    }
