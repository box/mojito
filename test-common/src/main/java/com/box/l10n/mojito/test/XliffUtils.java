...
public class XliffUtils {
  ...
  /**
   * Replaces content that changes from one test run to another (mostly db ids, dates, etc) in the
   * Xliff to allow simple comparison of the xliff content.
   *
  ...
  /**
   * Gets a function that call {@link #replaceXliffVariableContent(java.lang.String) } to modify
   * content.
   *
  ...
  /**
   * Localizes an Xliff file by replacing original targets by target where the content has been
   * prepended with a prefix and the locale.
   *
  ...
  public static String localizeTarget(String xliffContent, String localizedTargetPrefix) {
    return xliffContent.replaceAll(
        "<target xml:lang=\"(.*?)\" state=\"(.*?)\">(.*?)</target>",
        "<target xml:lang=\"$1\" state=\"$2\">" + localizedTargetPrefix + " - $3 $1</target>");
  }

  /**
   * Replaces the state of all targets in the XLIFF.
   *
   * <p>Note that the regex acutally replaces the state without looking at the target for context so
   * this assumes the state attribute is not used in another context (which should be fine for xliff
   * standard, if not the regex should be made more specific)
   *
   * @param xliffContent
   * @param state
   * @return
